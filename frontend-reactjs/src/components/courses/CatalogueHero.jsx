...
export default function CatalogueHero({ onSelectPlan, plans, currency, loading, error }) {
  const planEntries = Array.isArray(plans) && plans.length ? plans : DEFAULT_COURSE_PLANS;
  ...
  return (
    <div className="space-y-12">
      ...
      <div id="plan-comparison" className="space-y-6">
        ...
        {loading ? (
          <p className="text-sm text-slate-500">Loading latest plans…</p>
        ) : error ? (
          <p className="text-sm text-rose-600">{error}</p>
        ) : (
          <PlanComparison plans={planEntries} currency={currency} onSelectPlan={handlePlanSelect} />
        )}
      </div>
    </div>
  );
