...
function CourseHighlightCard({ course, onPurchase }) {
  const actions = Array.isArray(course.actions)
  ...
  const entity = {
    ...
    tags: Array.isArray(course.skills) ? course.skills : [],
    metrics: [
      { id: 'level', label: 'Level', value: course.level },
      { id: 'format', label: 'Format', value: course.deliveryFormat },
      { id: 'price', label: 'Price', value: course.price, tone: 'success' }
    ].filter((metric) => metric.value),
    media: {
      thumbnailUrl: course.thumbnailUrl,
      label: course.title,
      aspectRatio: course.aspectRatio ?? '4:3'
    },
    actions
  };

  const primaryAction = onPurchase
    ? { label: 'Purchase cohort', onClick: onPurchase, variant: 'primary' }
    : null;
