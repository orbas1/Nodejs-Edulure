          ...
          <ul className="mt-4 space-y-4">
            {upcoming.map((event) => (
              <li key={event.id} className="dashboard-card-muted p-4">
                ...
                <button
                  type="button"
                  className="mt-4 inline-flex items-center justify-center dashboard-primary-pill"
                >
                  {event.action}
                </button>
              </li>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {profileStats.map((stat) => (
              <div key={stat.label} className="dashboard-card-muted p-4">
                <p className="dashboard-kicker">{stat.label}</p>
                <p className="mt-2 text-xl font-semibold text-slate-900">{stat.value}</p>
                <p className={`mt-1 text-xs ${stat.trend === 'down' ? 'text-rose-500' : 'text-emerald-500'}`}>{stat.change} vs last
                  cycle</p>
              </div>
            ))}
          </div>
