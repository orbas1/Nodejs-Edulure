    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">Optimize spend, experiment with creative, and monitor ROAS in real time.</p>
        </div>
      ...
      <section className="dashboard-section">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {ads.active.map((campaign) => (
            <div key={campaign.id} className="dashboard-card-muted p-5">
              ...
              <p className="mt-1 text-sm text-slate-600">Spend {campaign.spend}</p>
              <p className="mt-2 text-xs text-emerald-500">{campaign.performance}</p>
      ...
      <section className="dashboard-section">
        ...
        <ul className="mt-4 space-y-4">
          {ads.experiments.map((experiment) => (
            <li key={experiment.id} className="dashboard-card-muted p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <p className="text-xs text-slate-600">{experiment.hypothesis}</p>
              </div>
            </li>
          ))}
        </ul>
