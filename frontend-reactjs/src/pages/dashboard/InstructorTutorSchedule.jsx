    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Ensure every mentor pod is resourced with the right availability.</p>
        </div>
        <button
          type="button"
      ...
      <section className="grid gap-4 md:grid-cols-2">
        {schedule.map((entry) => (
          <div key={entry.id} className="dashboard-panel">
            <div className="flex items-center justify-between text-xs text-slate-500">
              <span>{entry.mentor}</span>
              <span>{entry.slots}</span>
            </div>
            <p className="mt-3 text-sm text-slate-500">Learners assigned: {entry.learners}</p>
