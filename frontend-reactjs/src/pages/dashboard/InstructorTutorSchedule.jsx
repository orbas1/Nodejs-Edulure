        ...
        <div className="space-y-4">
          {state.entries.map((entry) => (
            <ScheduleCard
              key={entry.id}
              title={entry.mentor}
              subtitle={entry.learners || 'Unassigned learners'}
              statusLabel={entry.slots}
              statusTone="primary"
              metrics={[
                { label: 'Next availability', value: entry.nextAvailability || 'Sync calendar' },
                { label: 'Next session', value: entry.nextSession || 'None scheduled' },
                { label: 'Timezone', value: entry.timezone },
                { label: 'Location', value: entry.location }
              ]}
              tagGroups={[
                { tone: 'neutral', items: Array.isArray(entry.mediums) ? entry.mediums : [] },
                { tone: 'primary', items: Array.isArray(entry.tags) ? entry.tags : [] }
              ]}
              actions={[
                {
                  label: 'Edit',
              ...
              footer={
                entry.lastUpdatedAt
                  ? `Last updated ${new Date(entry.lastUpdatedAt).toLocaleString()}`
                  : null
              }
            >
