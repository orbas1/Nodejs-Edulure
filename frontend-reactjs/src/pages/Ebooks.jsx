...
export default function Ebooks() {
  ...
  const checkoutCurrency = checkoutEbook?.priceCurrency ?? 'USD';
  ...
  const checkoutPriceSummary = useMemo(() => {
    ...
    const unitAmountCents = Number(checkoutEbook.priceAmountCents ?? 0);
    ...
    const currency = checkoutEbook.priceCurrency ?? 'USD';
    const lineItems = [
      {
  ...
  const handleCheckoutSubmit = useCallback(
    async (event) => {
      ...
      try {
        const quantity = checkoutForm.quantity && checkoutForm.quantity > 0 ? checkoutForm.quantity : 1;
        const payload = {
        ...
        setCheckoutHistory((history) => [
          {
            ...
            amount: payment.amount ?? (checkoutEbook.priceAmountCents ?? 0) * quantity
          },
          ...history.slice(0, 9)
        ]);
      } catch (err) {
  ...
  return (
    <div className="bg-slate-100 pb-24 pt-10">
      ...
      <CheckoutDialog
        ...
        product={{
          title: 'Purchase marketplace download',
          subtitle: checkoutEbook?.title ?? null,
          description: checkoutEbook?.subtitle ?? checkoutEbook?.description ?? undefined,
          icon: CreditCardIcon
        }}
        ...
        {({ pending, blocking }) => (
          <>
            ...
            <label className="block space-y-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Copies</span>
              <input
                type="number"
                min="1"
                max="200"
