...
const overviewCache = createPersistentCache('finance-overview', { ttlMs: 5 * 60_000 });
const tenantCache = createPersistentCache('finance-tenants', { ttlMs: 60 * 60_000 });
...
export default function useFinanceDashboard({ pollIntervalMs = 180_000 } = {}) {
    ...
    async ({ silent = false } = {}) => {
      if (!isAuthenticated || !token) {
        setState((prev) => ({
          ...prev,
          loading: false,
          error: new Error('You must be authenticated to load finance metrics.')
        }));
        return;
      }
      ...
      try {
        const overview = await fetchFinanceOverview({ token, tenantId, signal: controller.signal });
  ...
  const refreshTenants = useCallback(async () => {
    ...
    try {
      const response = await fetchFinanceTenants({ token });
      if (!mountedRef.current) {
        return;
      }
  ...
  const value = useMemo(
    ...
    [refreshOverview, refreshTenants, state, tenantId, tenantState, updateData]
  );
