...
### Flow outline
- **Background jobs** – `startBackgroundJobs` in `backend-nodejs/src/servers/workerRoutines.js` wires ingestion, telemetry, moderation, and integration schedulers. `startWebServer` and `startWorkerService` bootstrap the shared runtime via `runtimeEnvironment.js`, then conditionally enable the runners using preset-aware toggles from `runtimeToggles.js`, keeping lite deployments lean while still exposing readiness probes on a dedicated HTTP server.【F:backend-nodejs/src/servers/workerRoutines.js†L1-L220】【F:backend-nodejs/src/servers/webServer.js†L1-L159】【F:backend-nodejs/src/servers/workerService.js†L1-L144】【F:backend-nodejs/src/servers/runtimeEnvironment.js†L1-L87】【F:backend-nodejs/src/servers/runtimeToggles.js†L1-L41】
- **Realtime gateway** – `attachRealtimeGateway` mounts Socket.IO namespaces for feeds, classrooms, and chat onto whichever HTTP server is running, so the consolidated `webServer.js` can co-host websockets while the dedicated `realtimeServer.js` path still exists for multi-process deployments.【F:backend-nodejs/src/servers/webServer.js†L30-L141】【F:backend-nodejs/src/servers/realtimeGateway.js†L1-L43】【F:backend-nodejs/src/servers/realtimeServer.js†L1-L135】
- **Search layer** – `searchProviders.js` registers a Postgres-backed provider (`postgresSearchProvider.js`) that proxies to `SearchDocumentModel`. Controllers and services (such as `ExplorerSearchService`) now resolve providers dynamically, leaning on the `search_documents` schema/migration plus seed rebuilders to keep catalogue metadata aligned without Meilisearch.【F:backend-nodejs/src/services/searchProviders.js†L1-L64】【F:backend-nodejs/src/services/search/providers/postgresSearchProvider.js†L1-L12】【F:backend-nodejs/src/models/SearchDocumentModel.js†L537-L604】【F:backend-nodejs/migrations/20250320140000_search_documents.js†L1-L87】【F:backend-nodejs/seeds/002_search_documents.js†L1-L37】【F:backend-nodejs/src/services/ExplorerSearchService.js†L1-L260】

### Assessments
A. ✅ **Redundancy Changes** – `runtimeEnvironment.js` now initialises database connections, feature flags, runtime config, and search polling once for every service; `webServer.js`, `workerService.js`, and `realtimeServer.js` all consume it while `stack.js` feeds the same preset-aware toggles, eliminating divergent bootstrap code paths across binaries.【F:backend-nodejs/src/servers/runtimeEnvironment.js†L1-L87】【F:backend-nodejs/src/servers/webServer.js†L1-L159】【F:backend-nodejs/src/servers/workerService.js†L1-L144】【F:backend-nodejs/src/servers/realtimeServer.js†L1-L135】【F:backend-nodejs/src/bin/stack.js†L1-L37】

B. ✅ **Strengths to Keep** – Scheduler registration still happens explicitly via `workerRoutines.js`, readiness probes expose per-component health, and the dedicated `realtimeServer.js` remains available so teams can scale to multi-process deployments without changing the orchestration contracts.【F:backend-nodejs/src/servers/workerRoutines.js†L1-L220】【F:backend-nodejs/src/servers/workerService.js†L1-L144】【F:backend-nodejs/src/servers/realtimeServer.js†L1-L135】

C. ✅ **Weaknesses to Remove** – The Postgres provider replaces Meilisearch by default, background jobs honour cooperative toggles to avoid double startups, and worker shutdowns cap resource usage by funnelling through the shared runtime cleanup path.【F:backend-nodejs/src/services/searchProviders.js†L1-L64】【F:backend-nodejs/src/services/search/providers/postgresSearchProvider.js†L1-L12】【F:backend-nodejs/src/services/ExplorerSearchService.js†L1-L260】【F:backend-nodejs/src/servers/workerService.js†L1-L144】

D. ✅ **Sesing and Colour Review Changes** – Job orchestration logs inherit the `[jobs]` prefix from `workerRoutines.js`, realtime gateway logging carries `[realtime]` via `realtimeGateway.js`, and readiness degradations surface consistent status copy that dashboards can colour-code predictably.【F:backend-nodejs/src/servers/workerRoutines.js†L30-L88】【F:backend-nodejs/src/servers/realtimeGateway.js†L1-L43】【F:backend-nodejs/src/servers/webServer.js†L47-L141】

E. ✅ **Improvements & Justification Changes** – Preset-driven toggles (`runtimeToggles.js`) determine whether jobs and realtime boot, and both `webServer.js` and `workerService.js` mark readiness degraded when features stay disabled. Search now flows through the provider registry, keeping observability intact while consolidating runtime entrypoints.【F:backend-nodejs/src/servers/runtimeToggles.js†L1-L41】【F:backend-nodejs/src/servers/webServer.js†L1-L159】【F:backend-nodejs/src/servers/workerService.js†L1-L144】【F:backend-nodejs/src/services/searchProviders.js†L1-L64】

F. ✅ **Change Checklist Tracker** – Vitest coverage locks in the new runtime contract: `webServer.test.js` validates embedded jobs/realtime with preset degradations, `workerService.test.js` exercises the standalone probe + toggle flow, and `searchProviders.test.js` protects provider registration/resolution. Schema, models, and seeds for `search_documents` stay in sync through migrations and the dedicated seed runner.【F:backend-nodejs/test/webServer.test.js†L1-L172】【F:backend-nodejs/test/workerService.test.js†L1-L181】【F:backend-nodejs/test/searchProviders.test.js†L1-L48】【F:backend-nodejs/migrations/20250320140000_search_documents.js†L1-L87】【F:backend-nodejs/seeds/002_search_documents.js†L1-L37】【F:backend-nodejs/src/models/SearchDocumentModel.js†L537-L604】

G. ✅ **Full Upgrade Plan & Release Steps** – Run the search document migration, reseed via `002_search_documents.js`, start the stack with `SERVICE_PRESET` tuned per environment, and execute the Vitest suite once dependencies install so operators can drop the standalone worker/realtime containers documented in `docker-compose.yml` for lite deployments.【F:backend-nodejs/migrations/20250320140000_search_documents.js†L1-L87】【F:backend-nodejs/seeds/002_search_documents.js†L1-L37】【F:backend-nodejs/src/bin/stack.js†L1-L37】【F:backend-nodejs/test/webServer.test.js†L1-L172】【F:backend-nodejs/test/workerService.test.js†L1-L181】
