...
function clampRotationInterval(value, provider) {
...
export default class IntegrationApiKeyService {
  ...
  async createKey({
    ...
    const hashConflict = await this.model.findByHash(keyHash);
  ...
  async rotateKey(id, { keyValue, rotationIntervalDays, expiresAt, rotatedBy, reason, notes }) {
  ...
  async disableKey(id, { disabledBy, reason }) {
    const record = await this.model.findById(id);
    if (!record) {
      throw Object.assign(new Error('API key not found'), { status: 404 });
    }
