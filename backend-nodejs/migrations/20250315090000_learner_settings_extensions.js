export async function up(knex) {
  ...
  const hasFinancePurchases = await knex.schema.hasTable('learner_finance_purchases');
  if (!hasFinancePurchases) {
    await knex.schema.createTable('learner_finance_purchases', (table) => {
      table.increments('id').primary();
      table
        .integer('user_id')
        .unsigned()
...
export async function down(knex) {
  await knex.schema.dropTableIfExists('learner_finance_purchases');
