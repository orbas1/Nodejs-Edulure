...
export async function up(knex) {
  ...
  if (!hasEvents) {
    await knex.schema.createTable('explorer_search_events', (table) => {
      ...
      table.json('filters').notNullable().defaultTo(JSON_EMPTY_OBJECT(knex));
      table.json('global_filters').notNullable().defaultTo(JSON_EMPTY_OBJECT(knex));
      table.json('sort_preferences').notNullable().defaultTo(JSON_EMPTY_OBJECT(knex));
