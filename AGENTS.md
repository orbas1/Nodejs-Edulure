...
9. **Database migrations & seeders**
   A. ✅ *Migrations* – Catalogue and schedule explicit migrations that unlock every UX surface: create `user_navigation_pins` (global shell pinning, Section 1), `marketing_hero_blocks` with media variants + CTA metadata (Section 2), extend `auth_identities` for passwordless + MFA envelopes (Section 3), add `learner_dashboard_snapshots` materialised view for overview cards (Section 4), introduce `course_viewer_assets` + `assessment_results` tables (Section 5), enrich `community_feed_items` with poll assets/ads slots (Section 6), append `live_session_streams` and `support_ticket_knowledge_base_links` junctions (Section 7), scaffold `field_service_orders` routing + `service_route_waypoints` (Section 8), and expand `creation_projects`, `instructor_tasks`, and `governance_compliance_alerts` (Section 9). Each migration must ship with down scripts, primary/foreign keys, JSONB metadata columns for future experiments, and indexed timestamps to keep dashboards responsive.
   B. ✅ *Seeders* – Author deterministic seed scripts mirroring the UX stories: pin default tabs for learner/instructor/admin personas, publish CMS-backed marketing heroes/testimonials, preload social login providers and MFA policies, craft sample onboarding journeys with saved responses, load learner dashboard cards with progress/upsell examples, seed flagship courses with modules, quizzes, certificates, and preview thumbnails, populate community feeds (posts, polls, ads placements, moderation flags), configure live session templates with transcript + replay data, generate concierge bookings across regions with follow-up surveys, and set up instructor workspaces with draft courses, monetisation tasks, governance warnings, and earnings history. Ensure assets reference S3/local blobs defined in `media_manifest.json` so screenshots and previews function in demos.
   C. ✅ *Maintenance* – Formalise recurring jobs mapped to UX health: nightly refresh + vacuum for materialised search/dashboard views, hourly sync of learner progress snapshots, weekly rebuild of marketing cache, continuous aggregation for community engagement + ads pacing, lifecycle hooks to expire live session replays, cron-driven concierge SLA checks, and governance alert escalations. Document each task in `scripts/maintenance/README.md`, wire to the internal scheduler preset (`SERVICE_TARGET=stack`), and add monitoring hooks so admin/operations consoles surface job status without manual queries.
