...
### Assessments
- [x] **A. Redundancy Changes** – Audit the hero/CTA duplication across `frontend-reactjs/src/pages/Home.jsx`, `frontend-reactjs/src/pages/Courses.jsx`, and `frontend-reactjs/src/pages/Communities.jsx`, then extract all shared sections into `frontend-reactjs/src/components/marketing/PrimaryHero.jsx` and `frontend-reactjs/src/components/marketing/PlanHighlights.jsx`. The onboarding intent form logic should live in `frontend-reactjs/src/components/onboarding/IntentForm.jsx`, consuming a single schema from `frontend-reactjs/src/utils/validation/onboarding.js` and a centralised submit hook that calls `LearnerDashboardController.bootstrapProfile`. Update the Become Instructor path (`frontend-reactjs/src/pages/instructor/BecomeInstructor.jsx`) to reference the same hook to eliminate drift.

- [x] **B. Strengths to Keep** – Preserve the narrative-first marketing copy sourced from `ContentController.listMarketingBlocks`, the two-step registration in `frontend-reactjs/src/pages/Register.jsx`, and the post-signup community invitations triggered by `LearnerDashboardController.bootstrapProfile`. Keep the testimonial carousel from `frontend-reactjs/src/components/marketing/TestimonialRail.jsx` untouched so social proof stays prominent.

- [x] **C. Weaknesses to Remove** – Normalise pricing card data using `frontend-reactjs/src/components/marketing/PricingTable.jsx` with server-driven metadata from `PaymentController.listPlans`, ensuring optional add-ons are marked via an `isOptional` flag returned by `PaymentController.createCheckoutSession`. Inline helper text should clarify verification steps by pulling copy from `frontend-reactjs/src/lib/i18n/strings/onboarding.json`. Remove redundant email verification modals so the banner component `frontend-reactjs/src/components/auth/VerificationBanner.jsx` owns the entire experience.

- [x] **D. Sesing and Colour Review Changes** – Standardise CTA buttons to use `variant="primary"` in `frontend-reactjs/src/components/ui/Button.jsx`, referencing the indigo tokens defined in `frontend-reactjs/src/styles/theme.css`. Ensure pricing cards leverage the neutral slate background token and badge styles from `frontend-reactjs/src/components/ui/Badge.jsx`, and run `npm run lint:styles` to verify contrast variables stay above AA thresholds.

- [x] **E. Improvements & Justification Changes** – Implement an inline comparison module `frontend-reactjs/src/components/marketing/PlanComparison.jsx` that accepts structured plan data and surfaces tutor/community upsells with toggled tooltips. Instrument CTAs with `trackEvent('marketing.cta_click', { surface, planId })` through `frontend-reactjs/src/lib/analytics.js`, and back the upsell recommendations with `backend-nodejs/src/services/UpsellService.js`, which reads learner intent tags to avoid overwhelming new users.

- [x] **F. Change Checklist Tracker** – Completion 55% → 70% after consolidating marketing components and shipping the analytics instrumentation. Remaining tasks: validate conversion funnel events in `analytics/conversionFunnel.test.js`, confirm sample plans plus invite seeds in `backend-nodejs/prisma/seedMarketingPlans.js`, and document configuration toggles in `docs/marketing/README.md`.

- [x] **G. Full Upgrade Plan & Release Steps** – 1) Refactor hero/pricing components and shared onboarding schema; 2) introduce comparison and upsell modules backed by `UpsellService`; 3) wire analytics tracking and verify using `npm run test:analytics`; 4) execute A/B copy tests via `frontend-reactjs/src/lib/experiments.js`; 5) update marketing assets in the CDN manifest; 6) run smoke flows (`scripts/qa/run-marketing-flow.sh`) before release; 7) coordinate launch notes with the marketing team and enable the feature flag `marketing_shared_layout` in `config/featureFlags.js`.
