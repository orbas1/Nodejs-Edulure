...
2. Part 2 — User Experience Audit & Component Breakdown
      ...
      - ✓ 8.B Dashboard Community & Messaging Suites
         ...
         - 8.B.10 useCommunityChatWorkspace (frontend-reactjs/src/pages/dashboard/community/instructorChats/useCommunityChatWorkspace.js)
           ...
           2. **Functionality.** `useCommunityChatWorkspace` memoises `socialGraph` metadata—nodes, edges, presence summary, role coverage—which powers ChannelSidebar, MessageTimeline, PresencePanel, RoleManagementPanel, and the composer without each component recomputing analytics.
           3. **Usefulness.** ChannelSidebar surfaces presence counts, top connectors, direct threads, and intro suggestions; MessageComposer introduces a direct-recipient picker; MessageTimeline displays peer counts and response cadence; PresencePanel summarises live status; RoleManagementPanel highlights active role coverage, fulfilling Annex A5 direct messaging & social graph requirements.
           ...
           5. **Placeholders.** Direct-thread membership defaults to empty when APIs do not expose participants; once Annex delivers explicit DM membership payloads we can swap the heuristics without changing the panels.
           ...
           17. **Back-end Integration.** The workspace hook now consumes channel, message, presence, role, event, and resource endpoints to synthesise Annex-aligned social graph data; once backend DM APIs land the same hook will populate direct-thread members and automate response-time analytics without further component rewrites.
           ...
           21. **Interact.** Direct-intro buttons call `onSelectDirectRecipient`, pre-filling the composer via `handleSelectDirectRecipient`, while aria-labels/data attributes expose analytics-friendly hooks and accessible descriptions for screen readers as mandated by `user_experience.md` Annex A5.
           22. **Missing.** Follow-on work should persist offline DM drafts, expose disabled states when no interactive session exists, and hydrate `directMessages.threads.members` from backend payloads once Annex A5 DM membership APIs solidify.
           ...
           25. **Framework.** `handleSelectDirectRecipient` partners with `useCommunityChatWorkspace.sendMessage` so metadata-enriched DMs respect existing capability guards, notice publishing, and token handling without bespoke plumbing.
