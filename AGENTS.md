...
10. **Alignment changes**
   ...
   B. ✓ *User experience* – Stitch every experience lane from `user experience.md` to its governing flow so delivery, QA, and design reviews speak the same language. Section 1’s global shell/navigation mandates (`AppTopBar`, `AppSidebar`, notification panel, analytics wiring) map to flows 1, 4, 5, and 11, ensuring stack presets, search migration, and dashboard analytics always ship with the shared navigation tokens, shortcuts, and persistence rules before marketing or dashboard updates proceed.【F:user experience.md†L1-L80】 Section 2’s marketing surfaces (heroes, testimonials, pricing ribbons) fuel flow 5 backlog items, while Section 3’s authentication/onboarding guidance grounds flow 5 + 7 deliverables so registration, identity, and monetisation cues reuse the same form kits, validation, and copy hooks.【F:user experience.md†L82-L153】 Section 4’s course consumption standards connect to flows 6 and 11 so viewer shells, progress widgets, and upsell rails move in tandem with learner lifecycle automation and analytics upgrades.【F:user experience.md†L155-L238】 Sections 5 and 6 (communities/social and live/support) anchor flows 8, 12, and 13 by demanding unified feed/composer primitives, moderation hooks, and realtime accessibility affordances whenever the operational playbooks shift.【F:user experience.md†L240-L347】 Sections 7–9 (field services concierge, instructor workspace, and subsequent operational appendices) align with flows 8, 9, and 14–16 so booking grids, technician logistics, instructor studios, and governance overlays inherit the same scheduling, analytics, and accessibility standards called out in their respective checklists before release approvals are granted.【F:user experience.md†L349-L452】 During sprint kickoff, insist that each squad links Jira/Linear tickets to both the flow ID and the UX section reference so design reviews trace tokens, component APIs, and copy decks back to the authoritative blueprint without divergence between web and lite-stack deliverables.
