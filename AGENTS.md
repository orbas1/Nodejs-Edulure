...
2. Part 2 — User Experience Audit & Component Breakdown
      ...
      - ✓ 8.A Public Community & Social Pages — Annex A20 (Community & Events)
        ...
        2. **Function.** Persona extraction, momentum scoring, and last-activity signals are centralised in `utils/communityPersona.js`; `Communities.jsx` feeds those metrics into a discovery grid while `Feed.jsx` renders a complementary “Community health” panel beside the live feed.
        3. **Usefulness.** Operators can slice communities by persona, access model, and momentum, see counts for open vs. NDA-gated cohorts, and jump directly into high-momentum chapters from both the discovery grid and the feed leaderboard.
        4. **Redundancies.** Prior hard-coded persona badges, repetitive list filters, and bespoke activity heuristics were replaced with shared helpers (`extractCommunityPersonas`, `computeCommunityEngagementScore`, `resolveLastActivity`) so both pages stay in sync.
        5. **Placeholders.** Backend APIs still return sparse persona metadata; the UI currently relies on heuristics (focus keywords, tag parsing) and should swap to explicit persona payloads once the services land.
        ...
        11. **Weaknesses to Remove.** Persona heuristics depend on keyword matching; upcoming backend changes should provide structured personas and monetisation metadata to avoid misclassification.
        ...
        22. **Missing.** Pending work includes wiring persona metrics into the API for SSR parity, adding saved filter presets per persona, and exposing aggregate charts for trend deltas beyond the top four personas.
