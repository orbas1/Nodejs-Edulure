...
2. Part 2 — User Experience Audit & Component Breakdown
      ...
      - ✓ 8.B Dashboard Community & Messaging Suites
        1. **Appraisal.** `InstructorCommunityChats.jsx` now binds the refreshed workspace hook, sidebar, composer, timeline, and presence cards so moderators can review broadcast channels, aggregated direct-message threads (with live previews), and social context in one responsive dashboard instead of juggling separate surfaces.
        2. **Functionality.** `useCommunityChatWorkspace.js` orchestrates every fetch—`listCommunityChannels`, `listCommunityMessages`, `listDirectMessages`, `sendDirectMessage`, `createDirectThread`, `markDirectThreadRead`, `listCommunityPresence`, `listCommunityEvents`, `fetchCommunityResources`, plus `fetchFollowers`/`fetchFollowing`/`fetchFollowRecommendations`/`fetchSocialMutes`/`fetchSocialBlocks`—and returns follow/unfollow helpers, social refresh callbacks, grouped channels, and memoised state consumed by ChannelSidebar, MessageComposer, MessageTimeline, and PresencePanel.
        ...
        5. **Placeholders or Stubs.** Direct-message kickoffs now auto-provision `/chat/threads` via `createDirectThread` when recommendation pills or composer recipients are used; remaining placeholders focus on exposing mute/block controls once those endpoints land, plus enriching DM analytics copy in the sidebar.
        ...
        7. **Improvements Needed.** Next steps wire mute/block mutations when backend routes ship, auto-refresh the social graph after DM sends, surface read-receipt badges in PresencePanel, and extend presence events so social summaries can call out on-duty moderators by role rather than raw counts.
        8. **Styling Improvements.** Sidebar metrics, recommendation chips, direct-message tiles, and timeline badges lean on the dashboard token set (`rounded-2xl`, `bg-slate-100`, `bg-primary/10`, uppercase kickers), yielding higher-contrast DM affordances while matching Annex typography and spacing rules.
        9. **Efficiency Analysis.** Memoised channel groups (`useMemo` around `channelGroups`/`directChannels`), set-based follower intersection checks, cached message pages, abortable loaders, and direct-thread read receipts keep large cohorts responsive even while presence, social graph, and DM data refresh in parallel.
        ...
        16. **Full Upgrade Plan & Release Steps.** Ship alongside `/social/*` and `/chat/*` endpoints, run `npm --workspace backend-nodejs run migrate && npm --workspace backend-nodejs run seed` to apply `20241123100000_community_chat.js`, `20250406120000_direct_message_threads_community_scope.js`, plus the updated `001_bootstrap.js` DM metadata, update Annex A5 screenshots with the sidebar/composer DM affordances, brief moderators on direct-message etiquette, and monitor follow/unfollow telemetry after launch.
        17. **Back-end Integration.** The hook now merges community chat APIs, social graph routes, and direct messaging models (`DirectMessageThreadModel`, `DirectMessageParticipantModel`, `DirectMessageModel`, `DirectMessageService`) so channel listings include per-thread membership, unread counts, `communityId` scoping, and latest previews seeded from the database rather than hard-coded mocks.
      ...
      - ✓ 8.C Community, Feed & Social Components
        ...
        2. **Functionality.** About panels persist quick links and spotlights via `usePersistentCollection`, the chat module normalises community channels and seeded direct threads (including unread counts + presence badges) with moderation + CRUD helpers backed by the `direct_message_threads.community_id` scope, CRUD/Member managers wrap admin APIs with staged forms, the interactive suite aggregates live/recorded classrooms, livestreams, podcasts, calendars, and leaderboards, while feed Composer/List/Card/SponsoredCard emit structured posts with attachment + sponsorship metadata.
