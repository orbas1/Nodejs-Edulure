...
2. Part 2 — User Experience Audit & Component Breakdown
      ...
      - ✓ 8.B Dashboard Community & Messaging Suites
        1. **Appraisal.** `InstructorCommunityChats.jsx` now binds the refreshed workspace hook, sidebar, composer, timeline, and presence cards so moderators can review channels and social context in one responsive dashboard instead of juggling separate surfaces.
        2. **Functionality.** `useCommunityChatWorkspace.js` orchestrates every fetch—`listCommunityChannels`, `listCommunityMessages`, `listCommunityPresence`, `listCommunityEvents`, `fetchCommunityResources`, plus `fetchFollowers`/`fetchFollowing`/`fetchFollowRecommendations`/`fetchSocialMutes`/`fetchSocialBlocks`—and returns follow/unfollow helpers, social refresh callbacks, grouped channels, and memoised state consumed by ChannelSidebar, MessageComposer, MessageTimeline, and PresencePanel.
        3. **Usefulness.** ChannelSidebar renders community selectors, active-channel metrics, direct-message groupings, and recommendation pills (with follow/DM controls) so instructors can escalate from broadcast updates to targeted outreach without leaving the dashboard.
        4. **Redundancies.** MessageComposer consolidates text, announcement, live-session, file, and direct-message flows with one schema for metadata (audience, priority, notifyFollowers/notifyFollowing, directRecipients), eliminating the bespoke DM and announcement forms that previously diverged in validation and copy.
        5. **Placeholders or Stubs.** Direct-message kickoffs still expect an existing DM channel ID; once the TODO around auto-provisioning direct threads (see `useCommunityChatWorkspace.js` TODO near the follow helpers) lands, recommendation pills can spin up new rooms without pre-seeded channels.
        6. **Duplicate Functions.** Social graph hydration, follow/unfollow mutations, channel normalisation, and metadata handling all live in the hook—downstream panels no longer duplicate HTTP clients, `Set` math, or message parsing to render badges and social chips.
        7. **Improvements Needed.** Next steps wire mute/block mutations when backend routes ship, auto-refresh the social graph after sending direct posts, and extend presence events so social summaries can call out on-duty moderators by role rather than raw counts.
        8. **Styling Improvements.** Sidebar metrics, recommendation chips, and timeline badges lean on the dashboard token set (`rounded-2xl`, `bg-slate-100`, `bg-primary/10`, uppercase kickers), yielding higher-contrast DM affordances while matching Annex typography and spacing rules.
        9. **Efficiency Analysis.** Memoised channel groups (`useMemo` around `channelGroups`/`directChannels`), set-based follower intersection checks, cached message pages, and abortable loaders keep large cohorts responsive even while presence and social data refresh in parallel.
        10. **Strengths to Keep.** EventPlanner, ResourceLibraryPanel, RoleManagementPanel, and moderation controls keep their existing flows but inherit centralised notices, capability checks, and composer metadata so no regressions hit scheduling or resource publishing.
        11. **Weaknesses to Remove.** DM workflows still depend on manual channel selection and lack mute/block plumbing; the outstanding TODOs cover auto-creating DM spaces, threading presence role data, and exposing moderation flags once APIs expand.
        12. **Styling & Colour Review.** MessageTimeline badges (“Followers pinged”, “Priority: priority”), PresencePanel summary chips, and composer controls share the neutral/primary/emerald palette already approved in Annex A5, keeping urgency signalling consistent across modules.
        13. **CSS, Orientation & Placement.** Sidebar sections retain fixed widths with scrollable bodies, the chat timeline hugs a flexible column beside the composer, and supporting panels stack in responsive grids (`xl:grid-cols-[320px_minmax(0,1fr)]`, `lg:grid-cols-3`) so layouts honour Annex breakpoint heuristics.
        14. **Text Analysis.** Workspace copy favours directive phrasing (“Sync”, “Direct”, “Select a direct channel”, “Message body required”) and short helper text, mirroring the operational tone documented in `user_experience.md` for Annex A5 social governance surfaces.
        15. **Change Checklist Tracker.** QA must cover: workspace refresh (channels/presence/resources/events/social graph), follow/unfollow notices, recommended connection CTAs, composer validation (direct channel guard, empty body), timeline metadata badges, and presence summary counts.
        16. **Full Upgrade Plan & Release Steps.** Ship alongside `/social/*` endpoints, update Annex A5 screenshots with the sidebar + composer metadata, brief moderators on direct-message etiquette, monitor follow/unfollow telemetry, and keep the DM auto-provision work flagged for the next sprint.
        17. **Back-end Integration.** The hook pulls from community chat APIs and social graph routes with shared token headers, centralising abort controllers and error handling so analytics/muted/block extensions can drop in without duplicating network layers.
        18. **Persona Coverage.** PresencePanel merges live session records with `socialGraphSummary` (followers/following/mutual/recommended) giving community managers, moderators, and instructors the situational awareness Annex A5 expects when balancing outreach.
        19. **Data Surfaces.** `socialGraphSummary` feeds sidebar chips and presence badges, composer metadata flows into MessageTimeline for moderation audits, and `workspaceNotice` provides inline alerts so analytics dashboards can later observe social actions without parsing UI state.
        20. **Notification & Broadcast.** Composer metadata flags (notifyFollowers/notifyFollowing, audienceSegment, priority, directRecipients, liveTopic/meetingUrl) are persisted with every post, letting downstream services decide who receives pushes versus silent drops per Annex broadcast rules.
        21. **Analytics Hooks.** Timeline badges, reaction buttons, and composer checkboxes provide deterministic selectors for instrumentation to log DM usage, follower pings, and moderation actions—setting the stage for Annex retention and escalation dashboards.
        22. **Governance & Compliance.** `ensureCapability` gates every mutation (posting, reacting, moderating, roles, events, resources, follow/unfollow) and emits descriptive notices so only authorised operators adjust social graphs or publish updates from the instructor console.
        23. **Accessibility.** Buttons keep semantic elements, focus-visible rings, and sr-only composer status updates; sidebar stats announce via live regions, ensuring keyboard/screen-reader operators can manage social workflows without friction.
        24. **Performance.** Direct-channel detection uses cached `Set` lookups, message pagination reuses `messageCache`, and abortable fetches prevent orphaned network calls when moderators swap communities mid-load.
        25. **Layout Cadence.** Rounded shells (`rounded-2xl`/`rounded-3xl`), 8px spacing rhythm, and consistent icon sizing align the sidebar, composer, timeline, and presence cards with dashboard design tokens so Annex reviewers see a unified system.
        26. **Release Guardrails.** `refreshWorkspace` + `refreshSocialGraph` return abort handlers for feature-flag rollouts, interactive mode toggles gate writes, and workspace notices surface failures—giving QA confidence when testing against staging APIs.
        27. **Telemetry & Feedback.** Centralised `workspaceNotice` success/error banners confirm follow/unfollow mutations, moderation changes, and composer validation, reducing the need for console logs while keeping Annex governance feedback loops explicit.
        28. **Developer Experience.** Consumers get a single hook signature (channels, messages, presence, roles, events, resources, social graph, notices, refreshers) so future Annex A5 iterations or tests can mock one provider rather than patching each panel independently.
      ...
      - ✓ 8.C Community, Feed & Social Components
        1. **Appraisal.** The Annex A4/B3 component suite spans identity (`CommunityAboutPanel.jsx`), chat operations (`CommunityChatModule.jsx`), admin tooling (`CommunityCrudManager.jsx`, `CommunityMembersManager.jsx`), programming (`CommunityEventSchedule.jsx`, `CommunityInteractiveSuite.jsx`, `CommunityMap.jsx`), and feed primitives (`components/feed/*.jsx`), so community operators can launch, programme, and broadcast from one toolkit.
        2. **Functionality.** About panels persist quick links and spotlights via `usePersistentCollection`, the chat module normalises channels/messages with moderation + CRUD helpers, CRUD/Member managers wrap admin APIs with staged forms, the interactive suite aggregates live/recorded classrooms, livestreams, podcasts, calendars, and leaderboards, while feed Composer/List/Card/SponsoredCard emit structured posts with attachment + sponsorship metadata.
        3. **Usefulness.** Operators can update community identity, share highlights, moderate chat, manage membership, seed events and resources, visualise programming calendars/maps, publish feed campaigns, and launch sponsored placements without wiring bespoke glue code for each surface.
        4. **Redundancies.** Normalisers (channel/message/member/resource/feed) and shared helpers (`resolveChannelType`, `normaliseMessageEntry`, avatar generators, tag formatters) centralise parsing logic so panels, directories, and cards consume consistent shapes.
        5. **Placeholders or Stubs.** Several widgets (CommunityMap coordinates, InteractiveSuite podcasts/livestreams, SponsoredCard analytics) still lean on seeded demo data—swap for production payloads once Annex data contracts land to avoid stale preview content.
        6. **Duplicate Functions.** Shared hooks (`usePersistentCollection`, `useCommunityRealtime`) and formatting utilities (date/number formatters, badge helpers) eliminate repeat state management while keeping UX parity across about, chat, resource, and feed experiences.
        7. **Improvements Needed.** Wire live map tiles + geocoding, replace seeded interactive suite data with backend feeds, expose bulk member actions + CSV exports, integrate SponsoredCard impression/click analytics, and sync Composer to backend audience targeting once Annex B3 APIs mature.
        8. **Styling Improvements.** Components standardise on `rounded-3xl`/`rounded-2xl` shells, uppercase kickers, pill badges, and Heroicons, mirroring Annex palette tokens (slate neutrals, primary/emerald highlights) across cards, forms, maps, and feed tiles.
        9. **Efficiency Analysis.** Memoised selectors, debounced search inputs, cached defaults, and lazy hydration (`useEffect` gating API calls) keep local previews snappy while allowing backend integration without thrashing renders.
        10. **Strengths to Keep.** Persistent drafts in About/Resource/Interactive panels, inline moderation controls in ChatModule, multi-step CRUD flows, and feed composer validation ensure forgiving workflows that encourage experimentation while preventing accidental publishes.
        11. **Weaknesses to Remove.** CommunityMap hardcodes coordinates + overlays, SponsoredCard lacks analytics instrumentation, and some resource editors still rely on manual asset IDs—prioritise backend wiring and telemetry guardrails in upcoming Annex passes.
        12. **Styling & Colour Review.** Role chips, member status pills, heatmap markers, leaderboard badges, and feed accents reuse the slate/primary/emerald palette plus subtle amber/rose warnings, honouring Annex accessibility ratios across dark text on neutral shells.
        13. **CSS, Orientation & Placement.** Responsive grids (`md:grid-cols-2`, `lg:grid-cols-3`, `xl:grid-cols-4`), sticky rails, scroll containers, and auto-flow cards keep long directories manageable while matching the layout heuristics in `user_experience.md` Annex A4/B3.
        14. **Text Analysis.** Copy favours concise, action-first language (“Assign moderation role”, “Launch retrospective”, “Invite via email”, “Publish resource”) and adds disclosure labels for sponsored content, ensuring global teams can scan interactions quickly.
        15. **Change Checklist Tracker.** QA should verify: quick-link/spotlight persistence, chat moderation + channel CRUD, schedule/event filtering, interactive suite tabs (live/recorded/podcasts/livestreams), map markers, member search/sort, feed composer validation, and sponsored disclosure toggles.
        16. **Full Upgrade Plan & Release Steps.** Phase backend integrations per module (identity APIs, programming feeds, analytics), refresh Annex screenshots, brief community ops on new programming dashboards, and monitor engagement + sponsorship metrics post-launch.
        17. **Back-end Integration.** Components already call `communityChatApi`, `communityApi`, and admin control routes; replacing seeds with production endpoints and hooking feed primitives into marketing CMS/analytics services will close the Annex B3 data story.
        18. **Persona Coverage.** AboutPanel surfaces admin/member counts + spotlights, MembersManager highlights moderators + invite paths, InteractiveSuite celebrates facilitators and operators, and feed components cater to marketing/monetisation owners—covering owners, moderators, and members.
        19. **Data Surfaces.** Event schedules, classrooms, livestreams, podcasts, calendars, resources, and feed campaigns surface structured metadata (tags, durations, statuses, visibility) in consistent cards for operators and analytics teams.
        20. **Notification & Broadcast.** Composer, ChatModule, and SponsoredCard emit payloads containing attachments, sponsorship disclosures, audience scopes, and CTA links, letting downstream services trigger pushes, emails, or curated placement slots without reverse engineering UI state.
        21. **Analytics Hooks.** Leaderboards expose ranks/point deltas, streak meters broadcast progress, feed items include impression-ready metadata, and member status badges highlight activation states—giving instrumentation deterministic selectors for Annex reporting.
        22. **Governance & Compliance.** CRUD + Member managers enforce visibility + role assignments, chat moderation guards enforce deletion/restoration flows, and resource editors capture ownership + tag metadata, aligning with Annex governance expectations.
        23. **Accessibility.** Forms include labels + descriptions, buttons keep semantic tags + focus states, carousels add aria-live hints, and maps/cards expose textual fallbacks so keyboard and screen-reader users can manage communities confidently.
        24. **Performance.** Local collections cache entries between sessions, chat normalisers batch updates, map/resource/interactive suites lazy-load seeded data, and feed lists paginate to keep community consoles responsive even with large datasets.
        25. **Layout Cadence.** Shared spacing rhythm (8px increments), consistent paddings, hero banners, and icon sizing keep community, programming, and feed surfaces visually aligned with dashboard modules and Annex brand guidance.
        26. **Release Guardrails.** Seeds + local persistence enable offline design reviews, feature flags isolate backend integrations, and admin forms validate required fields before network requests, giving teams safe rollout levers.
        27. **Telemetry & Feedback.** Inline success/error banners (“Saved link”, “Syncing chat”), hero icons, and progress indicators give immediate operator feedback while providing deterministic hooks for future analytics overlays.
        28. **Developer Experience.** Normalised prop-types, utility modules, storybook-friendly seeds, and shared hooks make it straightforward to test, extend, or reuse Annex components across upcoming community surfaces without duplicating scaffolding.
