...
2. **Redundancies**
   ...
   B. ✅ *User experience* – Mapped every repeated surface called out in `user experience.md` to the concrete files that need consolidation: marketing hero, testimonial, and pricing CTAs currently duplicated across Home, Courses, and Communities pages should all source the shared `frontend-reactjs/src/components/marketing/PrimaryHero.jsx`; the course/tutor/community cards rendered in Courses, Explorer, Dashboard, and Communities must fold into the `components/course/CourseCard.jsx` and `components/community/CommunityFeedList.jsx` primitives so ratings, monetisation ribbons, and media previews stay consistent; authentication and onboarding forms spread across login, register, instructor, and tutor flows should reuse `components/auth/AuthForm.jsx` with the validation logic in `utils/validation/auth.js`; community and live chat panels, countdown timers, and ticket drawers need to collapse into the `components/live/LiveChatPanel.jsx` and `components/support/TicketForm.jsx` scaffolds to prevent styling drifts; moderation queues, settings toggles, and analytics summary cards also require the shared layouts specified in the UX guide (`components/moderation/ModerationQueue.jsx`, `components/settings/SettingsLayout.jsx`, `components/admin/AdminSummaryCard.jsx`). Aligning these redundancies keeps navigation, accessibility tokens, and monetisation hooks identical across learner, instructor, admin, and support experiences.
