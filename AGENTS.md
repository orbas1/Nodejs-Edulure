...
8. **Component deletions**
   A. [x] *Targeted consolidation* – Schedule removal of bespoke hero banners (`frontend-reactjs/src/components/home/HomeHero.jsx`, `frontend-reactjs/src/components/marketing/CoursesHero.jsx`) and redundant dashboard summary cards described in Logic flow §§5–7 once the shared `PrimaryHero` kit and unified analytics cards are fully adopted. Tie each deletion to the rollout milestones in `logic flows.md` (learner acquisition, course consumption, learner profile) so teams retire the duplicates only after the refactored components render correctly across Home, Courses, Communities, DashboardHome, and Profile. Capture before/after screenshots in the UX changelog and update Storybook entries to reference the consolidated hero module.
   B. [x] *Search/UI overlap* – Decommission the alternate Explorer and Tutor grid implementations (`frontend-reactjs/src/components/explorer/ExplorerGridLegacy.jsx`, `frontend-reactjs/src/components/tutors/TutorCardLegacy.jsx`) once the Edulure Search provider from Logic flow §§2–4 powers the unified list primitives. Ensure the new grid consumes `useSearchProvider` hooks, supports preview metadata, and passes accessibility audits before removing the legacy bundles. Archive the legacy adapters under `frontend-reactjs/src/components/__deprecated__/search` for one release cycle, update routing fallbacks, and document the removal in the search migration notes.
   C. [x] *Performance tuning* – After the virtualised classroom/chat surfaces called out in Logic flow §§6–7 are stable, remove heavyweight carousel variants (`frontend-reactjs/src/components/media/HeroCarousel.jsx`, `frontend-reactjs/src/components/chat/ChatWindowLegacy.jsx`) and duplicate chat widgets embedded in community dashboards. Replace them with the shared virtualised list primitives backed by the realtime gateway (Logic flow §2) to reduce bundle size and idle CPU. Confirm Lighthouse scores stay above the accessibility/performance thresholds recorded in `user experience.md`, and migrate any bespoke analytics hooks before deleting the legacy components.
