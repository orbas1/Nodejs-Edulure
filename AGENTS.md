...
### Assessments
- [x] **A. Redundancy Changes** – Consolidate `CourseViewer`, `LearnerCourses`, and `DashboardHome` lesson navigation so they all mount the same `frontend-reactjs/src/components/courses/ModuleNavigator.jsx` primitive, sourcing state from the unified `useLearnerProgress` hook. Remove bespoke completion polling in `CourseViewer` and `LearnerTimelineWidget` by routing every update through `backend-nodejs/src/services/LearnerProgressService.js` and `CourseProgressController.syncProgress`, ensuring module completion, quiz attempts, and download tracking hit the identical reducer pipeline described in `logic flows.md` section 6.

- [x] **B. Strengths to Keep** – Preserve persisted progress snapshots, offline lesson caching provided by `CourseAssetCache` workers, and inline discussion entry points that link each lesson to its `CommunityChatController` thread. Keep the contextual “return to last lesson” ribbon that reads from `LearnerDashboardController.getActiveCourse` so the high-completion behaviour highlighted in the reference doc remains intact.

- [x] **C. Weaknesses to Remove** – Normalise quiz result messaging by moving score, rubric, and remediation copy into `frontend-reactjs/src/components/assessments/QuizOutcomePanel.jsx`, drawing strings from the shared i18n catalog. Streamline large-media handling by adding adaptive bitrate presets in `MediaUploadController` and teaching the player to preflight file size via `CourseMediaManifestModel`, eliminating the buffering regressions cited in the flow. Automate certificate issuance by reworking `AchievementsService.recordCompletion` to enqueue `CertificateGenerationJob` only after `LearnerProgressService` confirms all modules, preventing duplicate notifications and UI stalls.

- [x] **D. Sesing and Colour Review Changes** – Adopt the neutral slate background token (`surface.course`) for lesson canvases, lift the active module highlight to the primary 500 tone, and confine quiz feedback palettes to supportive green (`feedback.success`) and amber (`feedback.caution`) swatches. Ensure certificate previews use the print-safe palette and typography exported from `design-system/certificates.json` so on-screen and PDF outputs match the calm aesthetic mandated in the flow documentation.

- [x] **E. Improvements & Justification Changes** – Ship a reusable `ProgressStepper` component that renders module/lesson progress consistently across dashboard widgets and viewers, wired to `LearnerProgressService` deltas published over SSE. Integrate media compression presets (`720p`, `540p`, `audio-only`) surfaced via `MediaUploadController.describeAsset` so low-bandwidth learners can downgrade quality inline. Extend `frontend-reactjs/src/components/certification/CertificatePreview.jsx` to load template metadata from `AchievementsService.getCertificateTemplate`, giving immediate visual feedback and reducing manual support touches exactly as recommended in `logic flows.md`.

- [x] **F. Change Checklist Tracker** – Raise completion to 80% by covering regression tests for module navigation (happy path, branching modules), quiz submission flows, and certificate automation. Confirm database schema includes lesson completion audit tables (`LearnerLessonCompletionModel`) and achievements metadata; backfill seeds with two exemplar courses including certificate templates and diversified media assets to exercise the compression logic. Validate no new migrations beyond the achievements table are required and document deployment sequencing for progress service updates.

- [x] **G. Full Upgrade Plan & Release Steps** – Sequence delivery as: (1) land shared navigation and progress hook refactor; (2) deploy media manifest/compression support; (3) release unified quiz outcome messaging; (4) enable certificate template previews; (5) run end-to-end smoke tests covering enrollment → completion → certificate download; (6) update `logic flows.md` and onboarding docs; (7) monitor analytics dashboards (`CourseCompletionFunnel`) for regression before announcing general availability. Coordinate frontend and backend releases via feature flags so certificates and progress deltas remain consistent throughout the rollout.
