...
10. **Alignment changes**
   A. [x] *Logic flows* – Roadmap owners now map each of the sixteen flows to backlog epics, keeping stack orchestration (Flows 1–3) tied to the refactored `scripts/dev-stack.mjs` presets and `backend-nodejs/src/bin/stack.js` supervisor so rollout gates cover health checks, NDJSON logging, and cooperative scheduler tuning. Search and data portability tracks (Flows 4, 15, 16) run through the Postgres-first provider work, ensuring migrations, view refresh jobs, and analytics events are delivered alongside the API adapters. Learner/tutor lifecycle initiatives (Flows 5–12) carry shared acceptance criteria for achievements, booking orchestration, certificate automation, and checkout instrumentation, while the community/moderation coverage (Flows 8, 13, 14) maintains a single action ledger with undo stacks and compliance exports. Each stream now reports status back to the logic-flow index so engineering, data, and support can cross-check dependencies weekly.
   B. [x] *User experience* – Design system squads adopted the component kits and tokens referenced in `user experience.md`, pairing marketing, course consumption, communities, and admin consoles with the same PrimaryHero, summary card, settings layout, and accessibility primitives. Flow-specific UI touchpoints share figma specs, analytics IDs, and copy decks so CTAs, onboarding forms, pricing tiles, moderation checklists, and admin alerts behave consistently across responsive breakpoints. Accessibility and theming (Flow 17) now gate all releases, with focus order audits and colour contrast reviews run in lockstep with feature QA to prevent regressions when marketing or monetisation teams ship updates.
   C. [x] *Operational docs* – README, setup recipes, and runbooks reference the streamlined stack workflow, directing contributors to `logic flows.md` for context before touching code. The onboarding checklist walks through preset selection, Edulure Search provisioning, realtime toggles, and smoke tests, while the incident playbooks link to the moderation, ads, and analytics flows for escalation guidance. Contributor guidelines highlight where to update both Logic Flow and UX documents whenever new capabilities ship, keeping product, engineering, support, and revenue teams anchored to the same single source of truth.
