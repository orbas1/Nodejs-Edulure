...
8. **Component deletions**
   - [x] **A. Targeted consolidation** – Once the shared marketing/navigation kits from `user experience.md` sections 1–3 are live, delete legacy hero/banner implementations (`frontend-reactjs/src/components/marketing/HeroSlider.jsx`, `PrimaryHeroLegacy.jsx`, dashboard-specific hero stubs) that currently duplicate copy, CTA wiring, and asset preloading. Replace dashboard welcome banners in `LearnerDashboardHero.jsx` and `InstructorWelcomeCard.jsx` with the unified `PrimaryHero.jsx`/`DashboardSpotlightCard.jsx` pair so there is a single responsive source for imagery, monetisation badges, and analytics hooks. Archive the redundant SCSS modules tied to those deleted components so spacing and palette tokens centralise in `styles/tokens.css`.
   - [x] **B. Search/UI overlap** – After the unified search results grid outlined in sections 2, 5, 6, and 9 ships, retire the bespoke explorer/tutor list renderers (`ExplorerGridLegacy.jsx`, `TutorShowcaseCard.jsx`, `CourseSearchGridOld.jsx`) and route all list pages through the shared `SearchResultGrid.jsx` + `ResultCardComposite.jsx` primitives. Remove the parallel hook `useMeiliSearch.js` and its presentation wrapper; backfill tests to confirm `useSearch.js` drives Courses, Communities, Tutors, Library, and Marketing landing pages. Delete storybook stories and CSS tied to the deprecated grids so design reviews stay focused on the consolidated system.
   - [x] **C. Performance tuning** – Replace the heavy carousel/chat stacks called out in sections 6, 7, and 8 with the virtualised kits (`VirtualizedCarousel.jsx`, `LiveChatPanel.jsx`) and then drop the redundant components (`CommunityMediaCarouselLegacy.jsx`, `EventReplayCarousel.jsx`, `SupportChatWidgetStandalone.jsx`). Ensure marketing and dashboard sliders no longer import the deprecated Swiper wrappers; purge unused vendor bundles from `frontend-reactjs/src/vendor/` to reduce JS weight. Remove chat widget duplicates embedded in concierge/support layouts so live classrooms, support lounge, and concierge booking flows all depend on the shared chat primitive with a single socket manager.
