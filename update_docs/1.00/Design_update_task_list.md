...
- **Subtasks:**
  ...
  4. Specify card, form, and modal components including error/empty states and data placeholders (`Cards.md`, `Forms.md`). _Update:_ Booking confirmation, waitlist, cancellation, and recording consent modals now detail legal copy, countdown timers, and moderator overrides aligned to `Screen_text.md` and `Admin_panel_drawings.md`.
  5. Assemble imagery specifications and asset manifests referencing `images_and_vectors.md` and `Screens_update_images_and _vectors.md`. _Update:_ Payment method illustrations, finance alert icons, and VAT badge treatments approved for checkout/admin finance screens.
...
## Task 4 – Accessibility, Compliance & Handoff (Status: 38%)
...
- **Subtasks:**
  ...
  3. Compile security/privacy overlays for settings, payments, verification, consent prompts, and reinforce key rotation messaging aligned with backend JWT lifecycle. _Update:_ Email verification, lockout copy, antivirus/quarantine alerts, session governance (active devices, revoke-all confirmation), retention/audit banners, scheduler status/backoff chips, recording consent notices, tutor payout readiness steps, checkout PCI notices, Stripe/PayPal dispute messaging, and live class moderation prompts now mirror backend enforcement.
