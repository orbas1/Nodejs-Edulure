...
- Registered `/api/commerce` routes exposing order creation, capture, refund, and provider-specific webhook endpoints (`/stripe/webhook`, `/paypal/webhook`) with body-parser overrides to retain raw bodies for signature verification and role-aware guards for finance staff.
