...
- `backend-nodejs/migrations/20241127120000_admin_customer_service_panels.js`, models (`src/models/SupportTicketModel.js`, `SupportTicketEventModel.js`, `IncidentReportModel.js`, `AdminReviewRequestModel.js`, `PolicyDocumentModel.js`), service/controller/router (`src/services/AdminConsoleService.js`, `src/controllers/AdminConsoleController.js`, `src/routes/admin.routes.js`), OpenAPI schema updates (`src/docs/openapi.json`), Vitest suites (`test/adminConsoleService.test.js`, `test/adminHttpRoutes.test.js`), and seed enrichments inside `seeds/001_bootstrap.js` establish the admin/support console stack powering `/api/admin/console` snapshots with approvals, incidents, refunds, policy governance, and support backlog analytics.
