...
- `src/enablement/content/operator-onboarding-playbook.md` — enablement playbook detailing the three-day operator onboarding curriculum aligned with telemetry, monetisation, and support workflows. 【F:backend-nodejs/src/enablement/content/operator-onboarding-playbook.md†L1-L98】
- `src/enablement/content/analytics-revenue-enablement.md` — cross-functional analytics and revenue enablement curriculum referencing telemetry exports, revenue schedules, and storytelling practice. 【F:backend-nodejs/src/enablement/content/analytics-revenue-enablement.md†L1-L90】
- `src/enablement/content/stakeholder-communications-kit.md` — stakeholder communications kit with messaging templates, cadence governance, and escalation playbooks tied to platform metrics. 【F:backend-nodejs/src/enablement/content/stakeholder-communications-kit.md†L1-L87】
- `src/controllers/EnablementController.js` — Express controller exposing article listing, detail, capability matrix, and reindex endpoints backed by the enablement content service. 【F:backend-nodejs/src/controllers/EnablementController.js†L1-L78】
- `src/routes/enablement.routes.js` — Express router wiring the `/enablement` endpoints behind admin authentication. 【F:backend-nodejs/src/routes/enablement.routes.js†L1-L11】
- `src/services/EnablementContentService.js` — service that indexes markdown knowledge base content, renders HTML with sanitised fallbacks, caches metadata, and surfaces capability matrices. 【F:backend-nodejs/src/services/EnablementContentService.js†L1-L357】
- `test/enablementContentService.test.js` — Vitest coverage confirming indexing, filtering, markdown/HTML rendering, and capability matrix aggregation for enablement content. 【F:backend-nodejs/test/enablementContentService.test.js†L1-L56】
- `test/enablementHttpRoutes.test.js` — Supertest coverage validating `/enablement` HTTP routes, including listing filters, HTML rendering, reindex triggers, and capability matrix responses. 【F:backend-nodejs/test/enablementHttpRoutes.test.js†L1-L66】
- `migrations/20250305110000_governance_stakeholder_operations.js` — provisions governance contract, vendor assessment, review cycle, and roadmap communication tables with lifecycle metadata and indexes backing the new stakeholder cadence APIs. 【F:backend-nodejs/migrations/20250305110000_governance_stakeholder_operations.js†L1-L164】
- `src/models/GovernanceContractModel.js` — data mapper for governance contracts handling JSON obligations, lifecycle filtering, and renewal window aggregation. 【F:backend-nodejs/src/models/GovernanceContractModel.js†L1-L192】
- `src/models/GovernanceVendorAssessmentModel.js` — repository providing vendor risk lookup, status filtering, and remediation summary aggregation for assessments. 【F:backend-nodejs/src/models/GovernanceVendorAssessmentModel.js†L1-L191】
- `src/models/GovernanceReviewCycleModel.js` — review cycle accessor that normalises participants, action items, and milestone readiness scoring. 【F:backend-nodejs/src/models/GovernanceReviewCycleModel.js†L1-L192】
- `src/models/GovernanceRoadmapCommunicationModel.js` — roadmap communication persistence layer that tracks scheduling, metrics payloads, and attachments. 【F:backend-nodejs/src/models/GovernanceRoadmapCommunicationModel.js†L1-L176】
- `src/services/GovernanceStakeholderService.js` — orchestrates contract lifecycle summaries, vendor risk decisions, review actions, and communication scheduling while emitting governance metrics. 【F:backend-nodejs/src/services/GovernanceStakeholderService.js†L1-L212】
- `src/controllers/GovernanceController.js` — Express controller wiring admin governance endpoints with filter parsing, lifecycle updates, and decision handlers. 【F:backend-nodejs/src/controllers/GovernanceController.js†L1-L164】
- `src/routes/governance.routes.js` — Express router exposing `/governance` overview, contract, assessment, review, and communication endpoints behind admin authentication. 【F:backend-nodejs/src/routes/governance.routes.js†L1-L21】
- `test/governanceStakeholderService.test.js` — Vitest coverage validating overview aggregation, contract updates, vendor decisions, review actions, and communication scheduling. 【F:backend-nodejs/test/governanceStakeholderService.test.js†L1-L167】
- `test/governanceHttpRoutes.test.js` — Supertest coverage ensuring the governance HTTP routes enforce auth and surface stakeholder payloads. 【F:backend-nodejs/test/governanceHttpRoutes.test.js†L1-L120】
- `migrations/20250305113000_release_management.js` — provisions release checklist, run, and gate tables with lifecycle metadata powering the new release orchestration APIs. 【F:backend-nodejs/migrations/20250305113000_release_management.js†L1-L96】
- `src/models/ReleaseChecklistItemModel.js` — checklist repository exposing filtered listings and payload normalisation for release gates. 【F:backend-nodejs/src/models/ReleaseChecklistItemModel.js†L1-L168】
- `src/models/ReleaseRunModel.js` — release run data mapper handling filtering, pagination, and metadata serialisation for readiness orchestration. 【F:backend-nodejs/src/models/ReleaseRunModel.js†L1-L223】
- `src/models/ReleaseGateResultModel.js` — gate evaluation persistence layer supporting upserts, status summaries, and snapshot hydration. 【F:backend-nodejs/src/models/ReleaseGateResultModel.js†L1-L168】
- `src/services/ReleaseOrchestrationService.js` — orchestrates checklist retrieval, run scheduling, gate evaluations, readiness scoring, and Prometheus instrumentation for release management. 【F:backend-nodejs/src/services/ReleaseOrchestrationService.js†L1-L461】
- `src/controllers/ReleaseManagementController.js` — Express controller exposing release checklist, run, evaluation, and dashboard endpoints with envelope responses. 【F:backend-nodejs/src/controllers/ReleaseManagementController.js†L1-L139】
- `src/routes/release.routes.js` — Express router wiring the `/release` namespace behind admin authentication. 【F:backend-nodejs/src/routes/release.routes.js†L1-L13】
- `test/releaseOrchestrationService.test.js` — Vitest coverage validating schedule, evaluation, and metric emission flows for the release orchestration service. 【F:backend-nodejs/test/releaseOrchestrationService.test.js†L1-L174】
- `test/releaseHttpRoutes.test.js` — Supertest coverage ensuring release HTTP routes honour auth, surface readiness data, and invoke orchestration service contracts. 【F:backend-nodejs/test/releaseHttpRoutes.test.js†L1-L112】
