...
- Document the admin operational console blueprint referencing `Admin_panel_drawings.md`, `dashboard_drawings.md`, `menu_drawings.md`, and `Application_Design_Update_Plan/Application Design Update.md`: define subscription/community/support KPI tiles, approvals queue cards, incident bridges with severity badges, refund audit trays, support backlog pills with SLA countdowns, and policy governance grids so engineering can mirror `/api/admin/console` payloads without ad-hoc layout decisions.
