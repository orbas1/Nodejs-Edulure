...
- Document the Flutter shell architecture—Riverpod provider boundaries, authenticated navigation guards, feature flag gating, offline banners, and telemetry toasts—inside `App_screens_drawings.md`, `dashboard_drawings.md`, `menu_drawings.md`, and `Application_Design_Update_Plan/Application Design Update.md` so the mobile bootstrap mirrors the new engineering stack.
...
- Document the admin operational console blueprint referencing `Admin_panel_drawings.md`, `dashboard_drawings.md`, `menu_drawings.md`, and `Application_Design_Update_Plan/Application Design Update.md`: define subscription/community/support KPI tiles, approvals queue cards, incident bridges with severity badges, refund audit trays, support backlog pills with SLA countdowns, policy governance grids, and compliance workbench callouts (status badges, SLA copy, owner/contact metadata, export triggers) so engineering can mirror `/api/dashboard/me` admin payloads without ad-hoc layout decisions.
...
- Document identity verification flows across learner, instructor, and admin consoles: leverage `Admin_panel_drawings.md`, `dashboard_drawings.md`, `App_screens_drawings.md`, `Application_Design_Update_Plan/Application Design Update.md`, and `Design_Task_Plan_Upgrade/Web_Application_Design_Update/Function Design.md` to define required document checklists, upload progress treatments, checksum guidance, submission gating, approved/resubmission messaging, SLA timers, risk scoring badges, reviewer note fields, policy reference chips, and audit trail access so engineering can deliver compliant KYC interactions without reinterpreting legal requirements.
