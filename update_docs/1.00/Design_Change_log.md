...
- Finalised the admin operational console layouts in `Admin_panel_drawings.md`, `dashboard_drawings.md`, `menu_drawings.md`, and `Application_Design_Update_Plan/Application Design Update.md`: KPI tiles, approvals queue cards, incident bridges, refund audit trays, support backlog pills, and policy review grids now include spacing, typography, badge/urgency tokens, SLA countdown treatments, and escalation copy that match the new `/admin` React delivery and seeded `/api/admin/console` payloads.
- Documented compliance workbench refinements: policy hub cards now specify status badges, SLA annotations, owner/contact metadata, and runtime CTA behaviour while finance tiles enumerate export triggers and CSV schema hints referencing `Admin_panel_drawings.md`, `dashboard_drawings.md`, and `Design_Task_Plan_Upgrade/Web_Application_Design_Update/Web Application Design Update.md` so the refreshed `/admin` workspace mirrors policy governance and reporting expectations.
- Added identity verification workspace overlays: `dashboard_drawings.md`, `Admin_panel_drawings.md`, `App_screens_drawings.md`, and `Application_Design_Update_Plan/Application Design Update.md` now document learner/instructor verification cards (required document checklist, upload progress, checksum validation prompts, submission gating, approved/resubmission states) and the admin compliance queue (risk scoring badges, SLA timers, reviewer note validation, policy reference tags) so the React dashboards and admin console render production-grade KYC flows aligned with compliance/legal guidance.
...
- Finalised learner profile overview blueprints: hero metrics, verification badge rail, engagement insight tiles, programme shelves (communities, courses, ebooks), quick action ribbons, and timeline event stacks now include responsive spacing, accessibility notes, analytics tags, and cache-refresh cues aligned to `Profile Look.md`, `Profile Styling.md`, `dashboard_drawings.md`, `App_screens_drawings.md`, and `Admin_panel_drawings.md`, ensuring the React implementation mirrors production-ready design intent.
- Added notification centre trays, follower governance panels, and privacy toggles for learner dashboards using `dashboard_drawings.md`, `App_screens_drawings.md`, `menu_drawings.md`, and `Settings Dashboard.md` to capture toast hierarchy, approval/bulk actions, quiet-hour messaging, and preference audit trails now wired into the React delivery.
...
- Finalised instructor analytics dashboards in `dashboard_drawings.md`, `menu_drawings.md`, `Admin_panel_drawings.md`, and `Application_Design_Update_Plan/Application Design Update.md`: added course/tutor/live-classroom/tier revenue metrics, booking pipeline Kanban, ebook catalogue stats, ads experiment cards, pricing breakdown tables, and instructor search highlights so the refreshed `buildInstructorDashboard` payload maps 1:1 with production-ready layouts, copy, and accessibility annotations.
