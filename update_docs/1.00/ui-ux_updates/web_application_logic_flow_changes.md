...
## 16. Payments, Coupons & Refund Flow
1. Checkout surfaces itemised totals, tax, coupon entry, and wallet selection referencing `web_application_styling_changes.md` and `web_app_wireframe_changes.md`. Stripe card entry supports SCA with staged banners and fallback CTAs to PayPal or invoice workflows.
2. Coupon validation performs inline eligibility checks (status, usage caps, currency) with success/error copy sourced from `Design_Task_Plan_Upgrade/Web_Application_Design_Update/Web Application Design Update.md`; successful redemptions update totals and show audit breadcrumbs.
3. PayPal approvals redirect through hosted flow; upon return, UI polls webhook/intent status and surfaces receipt banners, download links, and escalation prompts if capture lags beyond SLA, mirroring guidance in `Application Design Update.md`.
4. Finance dashboards aggregate captured revenue, discounts, tax, and refunds with filter controls and export options referencing `Dashboard Designs.md` and `Design_Task_Plan_Upgrade/Application_Design_Update_Plan/Application Design Update.md`; exports trigger confirmation toasts and link to ledger audits.
5. Refund initiation (support/admin) includes amount selection, policy reminders, fraud escalation toggles, and role-based approval states. Confirmation screens echo backend statuses (pending, succeeded, failed) and provide webhook retry controls and support macros.
6. Stripe webhook failures generate notification centre entries and finance dashboard banners with retry buttons, SLA timers, and escalation contact references captured in `web_application_logic_flow_changes.md` to keep operators aligned with backend recovery paths.
