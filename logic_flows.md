...
6. **Duplicate Functions:** Each telemetry model implements near-identical `parseJson`/`parseArray` helpers (`TelemetryExportModel`, `TelemetryEventModel`, `TelemetryLineageRunModel`, `TelemetryConsentLedgerModel`), signalling the need for a shared utility to curb repeated parsing logic.
7. **Improvements need to make:** Extend environment scoping to telemetry ledgers by adding `environment_key` columns to `telemetry_event_batches`, `telemetry_events`, consent, and lineage tables, and persist those descriptors through `TelemetryExportModel` so multi-tenant dashboards can filter exports alongside alerts and forecasts.
...
11. **Weaknesses to remove:** `PlatformSettingModel.upsert` overwrites records without optimistic locking, and `TelemetryEventModel.markExportFailed` simply resets rows to `pending`; add version checks and explicit failure classifications so operators can separate retriable from terminal export errors.
...
15. **Change Checklist Tracker:** Before release, run analytics/governance migrations, reseed via `001_bootstrap`, execute telemetry warehouse job smoke tests, and validate `/api/v1/runtime/snapshot` plus the admin audit log feed to confirm runtime config and audit payloads reflect the updated schema.
16. **Full Upgrade Plan & Release Steps:** Apply migrations, backfill seeds, redeploy telemetry exporters, refresh runtime configuration caches, update Annex documentation/screenshots, coordinate compliance sign-off, and monitor alert plus export dashboards after launch.
...
4. **Redundancies:** Slug and JSON serialisation logic is scattered across blog, ebook, and ad models even though each performs similar sanitisation—consolidate helpers (slug casing, array serialisers, numeric coercion) to avoid divergent behaviour across marketing entities.
5. **Placeholders or non-working functions or stubs:** The schema references an `EnablementGuideModel`, yet enablement data currently lives in Markdown processed by `EnablementContentService`, and integration providers rely on the static `IntegrationProviderRegistry`; document these service-backed sources until dedicated tables are introduced.
6. **Duplicate Functions:** `EbookModel`, `AdsCampaignModel`, and `BlogPostModel` each ship bespoke `serialiseJson`/`recordView` patterns—promote shared utilities for JSON column writes, slugification, and metric increments to reduce duplication and simplify auditing.
7. **Improvements need to make:** Add localisation tables for multi-language posts/ebooks, persist enablement article usage analytics beyond the in-memory cache, track campaign channel breakdowns, and expose integration provider enablement via APIs so marketing dashboards show adoption in real time.
...
15. **Change Checklist Tracker:** Before shipping, run marketing/integration migrations, reseed catalogue/blog fixtures, refresh enablement indexes via the reindex endpoint, and validate campaign plus enablement API responses in staging.
16. **Full Upgrade Plan & Release Steps:** Stage schema updates, sync CMS exports, refresh enablement caches, brief marketing and enablement stakeholders, capture updated Annex screenshots, and monitor campaign plus enablement adoption metrics after deployment.
