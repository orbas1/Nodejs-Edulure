...
### 5.C Community, Social & Messaging Schema (`backend-nodejs/src/models/CommunityModel.js`, `backend-nodejs/src/models/CommunityEventModel.js`, `backend-nodejs/src/models/CommunityPostModel.js`, `backend-nodejs/src/models/CommunityMessageModel.js`, `backend-nodejs/src/models/DirectMessageThreadModel.js`, `backend-nodejs/src/models/CommunityDonationModel.js`, `backend-nodejs/src/models/CommunityAffiliatePayoutModel.js`, `backend-nodejs/src/services/ReactionAggregationService.js`, `backend-nodejs/migrations/20241115100000_community_core.js`, `backend-nodejs/migrations/20241123100000_community_chat.js`, `backend-nodejs/migrations/20241125140000_social_graph.js`, `backend-nodejs/migrations/20250322110000_community_post_reactions.js`)
1. **Appraisal:** The community cluster spans structural tables for `communities`, channels, resources, posts, events, and donations, layered with member/presence join tables and social-graph artefacts. `CommunityModel.listByUserWithStats` aggregates live counts for posts, resources, events, and activity recency, while Annex A40 seeds cover moderation-ready defaults, channel metadata, and persona tags.
2. **Functionality:** `CommunityPostModel` normalises tags, reaction summaries, moderation metadata, and pinned states; `CommunityEventModel` enforces timezone-aware scheduling, RSVP requirements, and recurrence hints; `CommunityMessageModel` drives threaded chat, attachment storage, and moderation visibility gates; and `DirectMessageThreadModel` plus `community_channel_members`/`user_presence_sessions` support DM routing, typing indicators, and delivery receipts. Donation and affiliate payout tables close the loop for community fund-raising and sponsorships, aligning Annex B3 monetisation hooks with social engagement data.
3. **Logic Usefulness:** ReactionAggregationService unifies counts coming from `community_post_reactions` and message reactions into the JSON `reaction_summary` envelopes returned by posts and feed services. Member helpers (`CommunityMemberModel.ensureMembership`) ensure permission snapshots stay aligned with cached feed payloads, while privacy tables (`user_privacy_settings`, `user_mute_list`, `user_block_list`) allow feed ranking and notification services to evaluate edges without additional joins.
4. **Redundancies:** Reaction totals live both as row-level entries (`community_post_reactions`, `community_message_reactions`) and as cached aggregates (`community_posts.reaction_summary`, `community_messages.metadata.reactions`), so reconciliation jobs must stay active to prevent drift. Presence data is duplicated between `user_presence_sessions` and cached dashboard snapshots, and both the channel and community membership tables track mute/notification flags—worth centralising to a shared read model.
5. **Placeholders Or non-working functions or stubs:** Event recurrence is stored as an iCal `recurrence_rule` string without expanding instances or linking to occurrence tables, so schedule orchestration remains TODO. `CommunityEventModel` captures lat/long but there is no geo-index or reverse-geocode integration, and chat message `message_type` enumerates `live`/`event` variants that lack the streaming hooks Annex A40 calls out.
6. **Duplicate Functions:** JSON parsing helpers (`parseJson`, `normaliseArray`) are re-implemented across `CommunityModel`, `CommunityPostModel`, `CommunityMessageModel`, and `DirectMessageThreadModel` even though `modelUtils.readJsonColumn`/`writeJsonColumn` exist; consolidating would make metadata handling consistent. Similar duplication exists for role/status coercion between `CommunityMemberModel` and `community_channel_members` repositories.
7. **Improvements need to make:** Partition high-volume entities (`community_messages`, `community_post_reactions`) by community or month, promote covering indexes for moderation filters (status + created_at + channel), and add a generated table for expanded recurrence instances tied to Annex live-programming analytics. A follow-up should expose windowed trending-materialised views so Explorer services stop re-counting post engagement on every request.
8. **Styling improvements:** Update ERDs and schema diagrams in Annex A40/B3 documentation to highlight colour-coded entity clusters (membership, messaging, monetisation) and include badges for encrypted columns, making it easier for design/docs consumers to map backend fields to UI treatments.
9. **Efficiency analysis and improvement:** Existing migrations already index posts by status, author, and publish dates and messages by channel/thread roots; we should extend this with partial indexes on `community_messages(status='visible')` and `community_post_reactions(reaction)` plus caching membership lookups in `CommunityModel.listByUserWithStats` to avoid repeated aggregate subqueries. Compressing large message payloads or moving attachments into `content_assets` would cut row size for hot channels.
10. **Strengths to Keep:** The schema enforces referential integrity with cascading deletes, tracks moderation actions (`community_message_moderation_actions`), encrypts sensitive DM payloads via `DataEncryptionService` fingerprints, and already surfaces stats (memberCount, activity timestamps) that power dashboards without additional joins.
11. **Weaknesses to remove:** Attachment references in posts/messages depend on manual cleanup when `content_assets` are removed, and donation/payment hooks can leave orphaned links when payment intents are pruned. Standardising cascades or background janitors plus ensuring `community_channel_members.last_read_message_id` is updated transactionally with message writes would reduce drift.
12. **Styling and Colour review changes:** Align Annex diagrams and UI mapping tables with the design-system tokens used for community badges (`docs/design-system/community.md`), differentiating posts vs. events vs. DMs through consistent hues that match `community_channels.channel_type` semantics.
13. **CSS, orientation, placement and arrangement changes:** Provide UI layout hints for representing multi-depth threads (root vs. replies), recurring event calendars, and social-graph suggestions so front-end shells can mirror backend groupings like `channel_type` or `user_follow_recommendations.reason_code` without bespoke heuristics.
14. **Text analysis, text placement, text length, text redundancy and quality of text analysis:** Expand schema docs to explain moderation status enums, reaction breakdown JSON structure, and privacy fields in plain language; avoid repeating “metadata” without examples by documenting keys inserted by services such as `CommunitySchedulingService` or `SupportKnowledgeBaseService` cross-posting.
15. **Change Checklist Tracker:** Every release should replay migrations `20241115100000_community_core.js`, `20241123100000_community_chat.js`, `20241125140000_social_graph.js`, and `20250322110000_community_post_reactions.js` in staging, verify websocket/event contract snapshots, re-run `npm --workspace backend-nodejs test -- community`, and confirm Annex A40/B3 documentation is regenerated.
16. **Full Upgrade Plan & Release Steps:** Apply migrations and reseed Annex payloads, warm caches for `CommunityService` and real-time subscriptions, execute load tests against hot channels/events, rehearse moderation queues with the new schema fields, update docs/screenshots, and ship with dashboard observability watching reaction reconciliation and DM delivery metrics.

### 5.D Commerce & Finance Schema (`backend-nodejs/src/models/PaymentIntentModel.js`, `backend-nodejs/src/models/PaymentRefundModel.js`, `backend-nodejs/src/models/PaymentLedgerEntryModel.js`, `backend-nodejs/src/models/CommunitySubscriptionModel.js`, `backend-nodejs/src/models/CommunityDonationModel.js`, `backend-nodejs/src/models/CommunityAffiliatePayoutModel.js`, `backend-nodejs/src/models/MonetizationReconciliationRunModel.js`, `backend-nodejs/migrations/20241112120000_payments_commerce.js`, `backend-nodejs/migrations/20250305103000_monetization_finance.js`)
1. **Appraisal:** The finance layer unifies payment intents, refunds, coupons, ledger entries, subscription lifecycle data, donations, affiliate payouts, usage metering, and reconciliation windows. Annex A41 cements governance by seeding catalog items, finance schedules, and initial reconciliation runs so dashboards and jobs observe realistic ledgers.
2. **Functionality:** `PaymentIntentModel` encrypts provider identifiers, receipt emails, and failure codes while exposing safe hashes for querying; `PaymentRefundModel` mirrors that approach for refunds; `PaymentLedgerEntryModel` records GAAP-friendly adjustments; `CommunitySubscriptionModel` persists customer period windows and provider statuses; and `CommunityDonationModel` plus affiliate payout models bind commerce to community campaigns. `MonetizationReconciliationRunModel` stores variance metrics that the reconciliation job reads to alert finance teams.
3. **Logic Usefulness:** Ledger and reconciliation tables give finance dashboards end-to-end traceability across capture, recognition, and payout events. Usage records and revenue schedules in `20250305103000_monetization_finance.js` power deferred revenue roll-forward calculations, while donation/payment intent joins enable Annex B4 to segment social fundraising performance against subscription ARR.
4. **Redundancies:** Status enums (`pending/succeeded/failed`) are duplicated across payment intents, refunds, donations, and payouts; aligning them in shared constants would prevent mismatched transitions. Similarly, amount/currency coercion logic is hand-rolled in each model rather than centralised, increasing the risk of rounding drift between ledger entries and reconciliation summaries.
5. **Placeholders Or non-working functions or stubs:** Usage metering currently allows null `catalog_item_id`/`payment_intent_id`, and no job populates the `monetization_usage_records.external_reference` or `revenue_schedules.recognition_end` beyond default timestamps. Tax handling is stubbed as JSON blobs with no schema validation, signalling future integrations with tax engines.
6. **Duplicate Functions:** Currency, amount, and metadata normalisers (`coerceAmount`, `normaliseCurrency`, JSON parse helpers) are copied between PaymentIntent, PaymentRefund, CommunityDonation, and PaymentLedgerEntry models; extracting them into `modelUtils` would trim duplication and guarantee matching precision.
7. **Improvements need to make:** Introduce double-entry journals that link ledger entries to GL account codes, expand reconciliation runs with automated acknowledgement workflows, and surface pending usage batches so finance can spot lagging ingestion. Building a view that joins catalog, usage, and schedules would give Annex dashboards a single source instead of recomputing totals in services.
8. **Styling improvements:** Refresh finance ERDs with palette cues for sensitive/encrypted columns, highlight deferred revenue flows, and provide UI layout specs for invoices, revenue schedules, and payout approval modals that pull directly from these tables.
9. **Efficiency analysis and improvement:** Existing migrations index intents by status/currency and usage by tenant/product/date; extend this with composite indexes on `payment_intents(entity_type, entity_id)` and partial indexes on `payment_refunds(status='pending')`. Partitioning high-volume usage and ledger tables by month would keep reconciliation scans fast, and batching ledger writes within `MonetizationReconciliationJob` reduces contention.
10. **Strengths to Keep:** Strong encryption via `DataEncryptionService`, audit timestamps across every table, seeded variance ratios for reconciliation analytics, and rich metadata blobs that allow downstream accounting exports to annotate reasons, sources, or affiliate codes without schema churn.
11. **Weaknesses to remove:** Manual ledger adjustments can still bypass approvals because `PaymentLedgerEntryModel.record` accepts arbitrary entry types—tie this to approval workflows and capture actor metadata. Donation refunds rely on manual status updates; wiring triggers from `PaymentRefundModel` would eliminate stale `community_donations.status` values.
12. **Styling and Colour review changes:** Finance documentation should mirror Annex B4 with high-contrast charts for revenue vs. variance, and UI teams need token guidance for representing statuses like `partially_refunded` or `pending` consistently across billing pages.
13. **CSS, orientation, placement and arrangement changes:** Supply grid/layout templates for financial statements, ARR dashboards, and payout queues so front-end apps can map `current_period_end`, `recognized_amount_cents`, and `payout_reference` to consistent table structures and badge placements.
14. **Text analysis, text placement, text length, text redundancy and quality of text analysis:** Clarify the distinction between `amount_total`, `amount_refunded`, and ledger adjustments in documentation, document allowable enum transitions, and ensure glossary entries differentiate invoices, intents, receipts, and donations to cut redundant phrasing in Annex copy.
15. **Change Checklist Tracker:** Run migrations `20241112120000_payments_commerce.js` and `20250305103000_monetization_finance.js` in staging, re-run finance Vitest suites, validate reconciliation dashboards against seeded data, confirm Stripe/PayPal sandbox credentials, and update Annex B4 screenshots before shipping.
16. **Full Upgrade Plan & Release Steps:** Coordinate schema deploys with billing freezes, backfill encryption hashes for legacy intents/refunds, warm reconciliation materialised views, notify finance ops of ledger changes, update customer comms templates, and monitor job metrics (`edulure_monetization_*`) plus payout queues post-release.
