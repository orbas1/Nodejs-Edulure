...
1. **Appraisal:** Billing now bridges Flutter clients to dedicated Node endpoints that publish the monetisation catalogue and queue receipt validations before finance reconciliation, reducing the risk of client-only drift.【F:backend-nodejs/src/controllers/BillingController.js†L22-L81】【F:backend-nodejs/src/services/BillingService.js†L52-L104】【F:backend-nodejs/src/models/BillingReceiptSubmissionModel.js†L75-L174】
2. **Functionality:** `BillingSyncService` hydrates `/billing/catalog` responses into cached `BillingCatalogSnapshot`s, while `CommercePaymentsController` surfaces pending receipt counts after each flush so learners see retry states mirrored in the UI.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L69-L198】【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L96-L153】
3. **Logic Usefulness:** Receipt submissions are deduplicated per transaction and normalised into JSON payloads, letting finance review seeds created via the new migration and bootstrap data without reprocessing duplicates.【F:backend-nodejs/src/models/BillingReceiptSubmissionModel.js†L75-L174】【F:backend-nodejs/migrations/20250327110000_mobile_billing_notifications.js†L11-L99】【F:backend-nodejs/seeds/001_bootstrap.js†L305-L355】
4. **Redundancies:** With catalogue metadata now served from `MonetizationCatalogItemModel`, clean up any remaining hard-coded plan descriptors in Flutter `CommercePaymentMethod` seeds once parity with backend copy is verified.【F:backend-nodejs/src/services/BillingService.js†L24-L48】【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L191-L205】
5. **Placeholders Or non-working functions or stubs:** Downgrade and proration previews remain TODO across mobile and backend; ensure feature flags or messaging acknowledge the gap before exposing plan change flows.【F:Edulure-Flutter/lib/services/commerce_models.dart†L1159-L1321】
6. **Duplicate Functions:** Currency/locale formatting still diverges between Flutter commerce helpers and backend reporting; extract a shared formatter (or expose via API) to avoid inconsistent currency precision.【F:Edulure-Flutter/lib/services/commerce_models.dart†L179-L333】
7. **Improvements need to make:** Layer analytics on the `/billing/receipts/validate` queue (success vs retry counts) and expose receipt audit details in the learner profile so pending payments are transparent.【F:backend-nodejs/src/controllers/BillingController.js†L47-L73】【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L136-L147】
8. **Styling improvements:** Extend paywall widgets with catalogue metadata (badge tone, landing links) now delivered in `BillingService.mapCatalogItemToProduct` so mobile layouts can mirror marketing collateral.【F:backend-nodejs/src/services/BillingService.js†L24-L48】
9. **Efficiency analysis and improvement:** Honour the 12-hour TTL already encoded in `BillingSyncService` by scheduling background refreshes and avoiding manual reload spam; consider etag support on `/billing/catalog` later for lighter payloads.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L69-L110】
10. **Strengths to Keep:** Offline-first receipt queueing, stateful catalogue caching, and persistent payment method hydration provide resilience against intermittent connectivity.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L117-L198】【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L96-L153】
11. **Weaknesses to remove:** Error feedback surfaced through `CommercePaymentsController` is still generic; map backend validation errors (`lastError` metadata) into learner-friendly copy and recovery hints.【F:backend-nodejs/src/models/BillingReceiptSubmissionModel.js†L118-L143】【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L136-L147】
12. **Styling and Colour review changes:** Sync badge tokens and plan accent colours between Flutter paywalls and the catalogue metadata inserted via seeders to maintain branded consistency.【F:backend-nodejs/seeds/001_bootstrap.js†L2916-L2988】
13. **CSS, orientation, placement and arrangement changes:** Continue optimising layouts for notched devices and landscape, ensuring catalogue metadata (e.g., feature lists) collapse gracefully in `BillingProduct` cards.【F:Edulure-Flutter/lib/services/commerce_models.dart†L1174-L1211】
14. **Text analysis, text placement, text length, text redundancy and quality of text analysis:** Audit plan copy now centralised in the catalogue to keep benefit statements concise and localisable without duplicating copy inside Flutter.【F:backend-nodejs/src/services/BillingService.js†L24-L48】
15. **Change Checklist Tracker:** Extend release checklists to include `/billing/catalog` smoke tests, receipt queue drains, and confirmation that `billing_receipt_submissions` rows reconcile with finance exports.【F:backend-nodejs/migrations/20250327110000_mobile_billing_notifications.js†L11-L111】【F:backend-nodejs/seeds/001_bootstrap.js†L305-L355】
16. **Full Upgrade Plan & Release Steps:** Rehearse catalogue refreshes in staging, monitor receipt queue metrics, push updated SDK docs, coordinate finance sign-off, then roll out with staged app distribution while observing refund/error telemetry.【F:backend-nodejs/src/services/BillingService.js†L52-L104】【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L69-L198】
...
1. **Appraisal:** Push, inbox, and support tooling now rely on Node APIs that persist preferences, register devices, and expose support cases as threaded conversations for mobile clients.【F:backend-nodejs/src/controllers/NotificationsController.js†L19-L74】【F:backend-nodejs/src/controllers/MobileInboxController.js†L19-L79】【F:backend-nodejs/src/services/NotificationPreferenceService.js†L36-L109】
2. **Functionality:** `PushNotificationService` still orchestrates Firebase registration while `NotificationPreferenceService` drains queued actions against `/notifications/preferences` and `/notifications/devices`, and `InboxService` maps support cases from `/inbox/threads` to local queues with retry metadata.【F:Edulure-Flutter/lib/services/push_notification_service.dart†L1-L138】【F:Edulure-Flutter/lib/services/notification_preference_service.dart†L400-L779】【F:Edulure-Flutter/lib/services/inbox_service.dart†L33-L189】
3. **Logic Usefulness:** Preferences are normalised and stored server-side with per-channel toggles, and learner replies now hydrate `LearnerSupportRepository` so server knowledge suggestions remain in sync with mobile drafts.【F:backend-nodejs/src/services/NotificationPreferenceService.js†L36-L109】【F:backend-nodejs/src/services/MobileInboxService.js†L60-L116】
4. **Redundancies:** With backend schemas in place, prune any lingering client-only preference caches once confidence grows in server responses to avoid stale channel toggles living exclusively in Hive storage.【F:Edulure-Flutter/lib/services/notification_preference_service.dart†L316-L509】
5. **Placeholders Or non-working functions or stubs:** SMS escalation remains a backlog item—guard UI affordances until backend routing supports that channel to prevent false expectations.【F:Edulure-Flutter/lib/services/notification_preference_service.dart†L612-L779】
6. **Duplicate Functions:** Harmonise support message formatting between mobile `InboxService` and the web dashboard so attachments and AI summaries render identically across surfaces.【F:Edulure-Flutter/lib/services/inbox_service.dart†L160-L320】【F:backend-nodejs/src/repositories/LearnerSupportRepository.js†L13-L109】
7. **Improvements need to make:** Extend preferences with quiet hours and per-category overrides surfaced by the new table so learners can mute channels without disabling push entirely.【F:backend-nodejs/migrations/20250327110000_mobile_billing_notifications.js†L51-L73】【F:backend-nodejs/src/services/NotificationPreferenceService.js†L61-L84】
8. **Styling improvements:** Use the newly persisted Slack workspace/channel metadata to prefill mobile UI, keeping badge colours and spacing aligned with design tokens across dark and light themes.【F:backend-nodejs/src/models/NotificationPreferenceModel.js†L49-L106】【F:Edulure-Flutter/lib/services/notification_preference_service.dart†L220-L314】
9. **Efficiency analysis and improvement:** Device registration now upserts on token; debounce additional client registrations and explore batching queued updates when reconnecting after long offline periods.【F:backend-nodejs/src/models/NotificationDeviceModel.js†L49-L106】【F:Edulure-Flutter/lib/services/notification_preference_service.dart†L520-L620】
10. **Strengths to Keep:** Offline inbox queuing, token lifecycle resilience, and support knowledge suggestions delivered from the repository provide a cohesive learner experience across unreliable networks.【F:Edulure-Flutter/lib/services/inbox_service.dart†L69-L189】【F:backend-nodejs/src/services/MobileInboxService.js†L60-L116】
11. **Weaknesses to remove:** Discovery of notification settings is still buried in account screens; leverage the `/notifications/preferences` summary to surface quick links in profile and support entry points.【F:backend-nodejs/src/controllers/NotificationsController.js†L19-L63】
12. **Styling and Colour review changes:** Ensure push badges, inbox unread pills, and support success banners adopt colours referenced in catalogue metadata for consistency once design reviews land.【F:backend-nodejs/seeds/001_bootstrap.js†L2916-L2988】
13. **CSS, orientation, placement and arrangement changes:** Continue refining inbox list density and support modal breakpoints so conversation threads seeded from the repository stay legible across phone and tablet orientations.【F:backend-nodejs/src/services/MobileInboxService.js†L60-L116】【F:Edulure-Flutter/lib/provider/communication/communication_store.dart†L40-L118】
14. **Text analysis, text placement, text length, text redundancy and quality of text analysis:** Use stored AI summaries and knowledge suggestions to keep mobile support copy concise and empathetic instead of duplicating phrasing locally.【F:backend-nodejs/src/repositories/LearnerSupportRepository.js†L13-L110】
15. **Change Checklist Tracker:** Add preference API smoke tests, device registration audits, and inbox message replay checks to mobile release gates alongside push delivery verification.【F:backend-nodejs/migrations/20250327110000_mobile_billing_notifications.js†L51-L99】【F:backend-nodejs/src/controllers/MobileInboxController.js†L19-L79】
16. **Full Upgrade Plan & Release Steps:** Stage notification preference migrations, replay inbox queues in staging, update runbooks for Slack test events, then roll out incrementally while observing push token churn and support response times.【F:backend-nodejs/src/controllers/NotificationsController.js†L19-L74】【F:backend-nodejs/src/controllers/SlackIntegrationController.js†L1-L32】
...
1. **Appraisal.** `CommercePaymentsController` now brokers billing state across cached payment methods, catalog metadata, and receipt queues by delegating to the new `BillingSyncService`, giving the mobile layer parity with reconciliation pipelines.【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L1-L210】【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L1-L213】
2. **Functionality.** During bootstrap the controller hydrates stored methods, eagerly refreshes the billing catalog, and triggers receipt synchronisation while exposing convenience methods for forced refreshes and new purchase queuing.【F:Edulure-Flutter/lib/provider/commerce/commerce_payments_controller.dart†L54-L142】
3. **Logic Usefulness.** `BillingSyncService` centralises catalog TTL evaluation, guarded receipt flushes, and network calls so UI flows can reason about freshness and retry schedules without duplicating protocol knowledge.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L19-L154】
4. **Redundancies.** Currency and plan metadata formatting still live inside both `BillingProduct` and `CommunitySubscriptionPlan`; consolidating these to shared helpers would curb divergence when marketing copy changes.【F:Edulure-Flutter/lib/services/commerce_models.dart†L1000-L1251】
5. **Placeholders or non-working functions.** API routes such as `/billing/catalog` and `/billing/receipts/validate` remain stubbed until backend endpoints stabilise, so error reporting currently falls back to cached data without user-visible notices.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L63-L118】
6. **Duplicate Functions.** Receipt persistence and general commerce persistence both encode JSON lists; future refactors could share encoder utilities to avoid maintaining parallel serialisation code paths.【F:Edulure-Flutter/lib/services/commerce_models.dart†L1176-L1213】【F:Edulure-Flutter/lib/services/commerce_persistence_service.dart†L1-L155】
7. **Improvements needed.** Wire webhook listeners into the mobile cache invalidation hooks, expose renewal grace-period banners, and surface product metadata like usage caps inside the learner subscription UI.
8. **Styling improvements.** Update plan cards within `community_subscription_screen.dart` to surface billing cycle badges sourced from `BillingProduct.billingCycle`, and align button contrast with accessibility tokens.
9. **Efficiency analysis and improvement.** Catalog fetches now respect a twelve-hour TTL and receipt submissions throttle to five-minute windows, but seeding still loads full subscriber lists—paginate invoices before scaling cohorts.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L44-L154】【F:Edulure-Flutter/lib/provider/commerce/community_subscription_controller.dart†L1-L200】
10. **Strengths to keep.** The persistence layer abstracts Hive storage so catalog snapshots, receipts, and payment methods share consistent error handling and crash-safe writes.【F:Edulure-Flutter/lib/services/commerce_persistence_service.dart†L1-L155】
11. **Weaknesses to remove.** Manual seed data in controllers masks real catalog gaps and may obscure entitlement discrepancies when the backend diverges—replace seeds with explicit onboarding tours.
12. **Styling and colour review changes.** Ensure subscription detail screens distinguish trial states with semantic colours tied to the design tokens introduced for billing status chips.
13. **CSS, orientation, placement and arrangement changes.** Reflow plan comparison grids into vertically stacked cards on mobile while retaining multi-column layouts for tablets to keep new pricing metadata readable.
14. **Text analysis, placement, length, redundancy and quality.** Rewrite plan descriptions to highlight renewal cadence and tax handling, remove redundant mentions of “auto-renewal” in adjacent paragraphs, and link to refund policies.
15. **Change checklist tracker.** Actions include instrumenting catalog freshness analytics, validating receipt retries against sandbox processors, localising plan copy, and aligning finance runbooks with the queued receipt workflow.【F:Edulure-Flutter/lib/services/billing_sync_service.dart†L104-L154】
16. **Full upgrade plan & release steps.** Phase 1 deploys the shared sync service, Phase 2 aligns UI copy/styling, Phase 3 connects live webhooks and pagination, and Phase 4 runs finance sign-off with end-to-end sandbox purchase rehearsals.

### A31. Flutter Notifications, Messaging & Support (3.F)
1. **Appraisal.** Notifications now coordinate token lifecycle management with inbox synchronisation so device registration, foreground display, and queued outbound messages operate cohesively across push and chat modules.【F:Edulure-Flutter/lib/services/push_notification_service.dart†L1-L209】【F:Edulure-Flutter/lib/services/inbox_service.dart†L1-L238】
2. **Functionality.** `PushNotificationService` initialises Firebase, provisions local notifications, registers listeners, and serialises token registration through a guarded `_registerToken`, while `InboxService` fetches threads, queues outbound messages, and flushes pending payloads when connectivity returns.【F:Edulure-Flutter/lib/services/push_notification_service.dart†L34-L157】【F:Edulure-Flutter/lib/services/inbox_service.dart†L33-L190】
3. **Logic Usefulness.** The receipt-style `_syncInFlight` guards inside `InboxService` prevent duplicate submissions, and the new pending message queue persists retries alongside metadata (retry count, last error) to aid support diagnostics.【F:Edulure-Flutter/lib/services/inbox_service.dart†L92-L189】【F:Edulure-Flutter/lib/provider/communication/communication_models.dart†L1-L165】
4. **Redundancies.** Offline persistence now stores pending support tickets and inbox messages separately; consider collapsing both into a general outbox abstraction so retry logic and throttling live in a single module.【F:Edulure-Flutter/lib/services/communication_persistence_service.dart†L1-L155】
5. **Placeholders or non-working functions.** The inbox sender currently assumes `/inbox/threads/{id}/messages` returns the delivered payload; until the backend mirrors that contract, sent messages fall back to optimistic copies marked `sent` without server enrichment.【F:Edulure-Flutter/lib/services/inbox_service.dart†L191-L238】
6. **Duplicate Functions.** Token registration throttling exists in both `PushNotificationService` and `NotificationPreferenceService`; factoring a shared utility would avoid drift between refresh cadence and cache rules.【F:Edulure-Flutter/lib/services/push_notification_service.dart†L91-L148】【F:Edulure-Flutter/lib/services/notification_preference_service.dart†L520-L602】
7. **Improvements needed.** Surface offline banner states in chat UIs when `_syncInFlight` is active, expose retry controls to agents, and hook inbox fetches into background isolates for long-lived listeners.
8. **Styling improvements.** Update support and inbox views to badge pending messages with subtle status pills and ensure notification preference toggles communicate queued sync status using consistent iconography.
9. **Efficiency analysis and improvement.** Token refreshes are serialised to eliminate race conditions and inbox retries back off by two minutes, but background sync should adopt exponential backoff when retries exceed thresholds.【F:Edulure-Flutter/lib/services/push_notification_service.dart†L91-L148】【F:Edulure-Flutter/lib/services/inbox_service.dart†L118-L189】
10. **Strengths to keep.** Hive-backed persistence abstracts thread caching, queued messages, and support tickets, giving the mobile app resilient offline behaviour even when transport fails.【F:Edulure-Flutter/lib/services/communication_persistence_service.dart†L1-L155】
11. **Weaknesses to remove.** Notification copy, support macros, and chat tooltips still live in disparate locales—centralise strings so messaging remains consistent across push, inbox, and ticketing.
12. **Styling and colour review changes.** Ensure push notification previews and in-app banners adopt accessible contrast levels and respect dark-mode palettes defined for support toasts.
13. **CSS, orientation, placement and arrangement changes.** Reserve vertical space for offline banners on small devices, collapse message composer toolbars when retry queues are active, and anchor notification preference summaries near the top of the settings view.
14. **Text analysis, placement, length, redundancy and quality.** Audit support automation copy to remove redundant escalations, streamline push notification text to front-load action verbs, and add inline definitions for severity levels.
15. **Change checklist tracker.** Track token refresh QA, verify queued message replay after reconnect, exercise support ticket persistence, and document recovery steps for ops teams using the new queue metadata.【F:Edulure-Flutter/lib/services/push_notification_service.dart†L91-L157】【F:Edulure-Flutter/lib/services/inbox_service.dart†L118-L238】
16. **Full upgrade plan & release steps.** Stage deployment with refreshed Firebase credentials, run messaging regression tests, roll out offline banner UX updates, and conclude with coordinated support training on managing queued conversations.
