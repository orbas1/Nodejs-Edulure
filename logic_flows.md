...
2. **Functionality:** The helper script validates Docker prerequisites, executes `docker compose --profile observability up -d`, waits for readiness, and surfaces login hints; provisioning manifests auto-load dashboards while `backend-nodejs/src/observability/metrics.js` and `sloRegistry.js` feed live telemetry.
...
15. **Change Checklist Tracker:** Release readiness now includes running `npm run dev:observability`, validating `/metrics` auth, confirming SLO annotations, and capturing dashboard screenshots for Annex artefacts.
...
15. **Change Checklist Tracker:** Each release should confirm the onboarding script passes, docs reference current ports/credentials, and file listings (`file_list.md`) include new helper scripts.
