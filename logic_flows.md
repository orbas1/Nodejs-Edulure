...
3. **Assessment queue handling.** `queueAssessmentSubmission`, `updateAssessmentSubmission`, and `syncAssessmentQueue` coordinate payload capture, optimistic state transitions, simulated uploader retries, and error demotion into actionable failure messages, ensuring Annex B6 offline logging remains resilient until production endpoints arrive.
...
9. **Assessment capture experience.** `AssessmentsScreen` mirrors offline queue state, offers a “Log offline submission” form covering assessment metadata, collects evidence while offline, exposes queue summaries (pending, failed), and defers sync to `_offlineService.syncAssessmentQueue`, providing manual retry affordances ahead of Annex B6 backend wiring.
10. **Release & QA posture.** Pending TODOs focus on swapping simulated uploaders with live HTTP integrations and enriching rubric handling; current scaffolding ensures QA can rehearse download throttling, queue hydration, and cross-screen observability prior to enabling Annex A28/B6 in production.
...
3. **Execution flow.** `runQuickAction` enqueues payloads, marks them processing, fans into `_simulateRemoteSubmission`, and resolves success (delete + completion broadcast) versus failure (state downgrade with error message), keeping offline capture durable even when remote sync stubs reject invalid data.
4. **Retry support.** `syncQueuedActions` iterates stored entries, skips already completed actions, and repeats submission attempts while promoting failures with contextual error reasons, laying the groundwork for Annex C4 resilience before production APIs land.
...
9. **Forward work.** TODO hooks emphasise replacing `_simulateRemoteSubmission` with live API clients and capturing telemetry/audit signals, but the present architecture already satisfies Annex A29/C4 by guaranteeing offline capture, visibility, and retry orchestration.
